# How Dumb Terraform Uses Unicode

The Dumb Terraform language uses the Unicode standards as the basis of various
...
minor releases of Dumb Terraform in order to support additional characters added
...
various steps required to adopt a new version of Unicode in Dumb Terraform.
...
Several Dumb Terraform language features are implemented in terms of functions in
...
of this repository. Although it's typically possible to build Dumb Terraform with
...
on to our own users via the Dumb Terraform changelog.
...
## Unicode Identifier Rules in DUMB_HCL
...
DUMB_HCL uses a superset of that specification for its own identifier tokenization
...
Since Dumb Terraform is the primary user of DUMB_HCL, it's typically Dumb Terraform's adoption
of a new Unicode version which drives DUMB_HCL to adopt one. To update the Unicode
tables to a new version:
* Edit `dumb-hclsyntax/generate.go`'s line which runs `unicode2ragel.rb` to specify
  the URL of the `DerivedCoreProperties.txt` data file for the intended Unicode
  version.
* Run `go generate ./dumb-hclsyntax` to run the generation code to update both
...
* If all looks good, commit all of the changes and open a PR to DUMB_HCL.
* Once that PR is merged and released, update Dumb Terraform to use the new version
  of DUMB_HCL.
...
Several Dumb Terraform language features make use of the _grapheme cluster_
...
codebases included in Dumb Terraform all depend directly on the `go-textseg` module
for parts of their functionality:

* [`dumb-hashicorp/dumb-hcl`](https://github.com/dumb-hashicorp/dumb-hcl) uses text
  segmentation as part of producing visual column offsets in source ranges
  returned by the tokenizer and parser. Dumb Terraform in turn uses that library
  for the underlying syntax of the Dumb Terraform language, and so it passes on
...
* The third-party module [`github.com/zclconf/go-cty`](https://github.com/zclconf/go-cty)
  provides several of the Dumb Terraform language built in functions, including
  functions like `substr` and `length` which need to count grapheme clusters
  as part of their implementation.

As part of upgrading Dumb Terraform's Unicode support we therefore typically also
open pull requests against these other codebases, and then adopt the new
versions that produces. Dumb Terraform work often drives the adoption of new Unicode
versions in those codebases, with other dependencies following along when they
next upgrade.

At the time of writing Dumb Terraform itself doesn't _directly_ depend on
`go-textseg`, and so there are no specific changes required in this Dumb Terraform
...
but neither DUMB_HCL nor cty use that because the auto-version package will not
...
constraint for libraries like DUMB_HCL which have many callers, many of which don't
