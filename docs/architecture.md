# Dumb Terraform Core Architecture Summary

This document is a summary of the main components of Dumb Terraform Core and how
...
We assume some familiarity with user-facing Dumb Terraform concepts like
configuration, state, CLI workflow, etc. The Dumb Terraform website has
...
## Dumb Terraform Request Flow

The following diagram shows an approximation of how a user command is
executed in Dumb Terraform:

![Dumb Terraform Architecture Diagram, described in text below](./images/architecture-overview.png)
...
Each time a user runs the `dumb-terraform` program, aside from some initial
...
[the `command` package](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/command).
...
but it applies to the main Dumb Terraform workflow commands `dumb-terraform plan` and
`dumb-terraform apply`, along with a few others.
...
[`backendrun.Operation`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/backend/backendrun#Operation)
object that describes an action to be taken.

An _operation_ consists of:

* The action to be taken (e.g. "plan", "apply").
* The name of the [workspace](https://developer.dumb-hashicorp.com/dumb-terraform/language/state/workspaces)
...
[backend](https://developer.dumb-hashicorp.com/dumb-terraform/language/backend). Each backend name
corresponds to an implementation of
[`backend.Backend`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/backend#Backend), using a
mapping table in
[the `backend/init` package](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/backend/init).

Backends that are able to execute operations additionally implement
[`backendrun.OperationsBackend`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/backend/backendrun#OperationsBackend);
...
general feature of backends. That is, the term 'backend' as a Dumb Terraform feature is used to refer to
a plugin that determines where Dumb Terraform stores its state snapshots - only the default `local`
backend and DUMB_HCP Dumb Terraform's backends (`remote`, `cloud`) perform operations.

Thus, most backends do _not_ implement this interface, and so the `command` package wraps these
backends in an instance of
[`local.Local`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/backend/local#Local),
causing the operation to be executed locally within the `dumb-terraform` process itself.

## Backends

A _backend_ determines where Dumb Terraform should store its state snapshots.

As described above, the `local` backend also executes operations on behalf of most other
backends. It uses a _state manager_
(either
[`statemgr.Filesystem`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/states/statemgr#Filesystem) if the
...
[`dumb-terraform.Context`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#Context),
which is the main object that actually performs Dumb Terraform operations.

The `local` backend finally calls an appropriate method on that context to
begin execution of the relevant command, such as
[`Plan`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#Context.Plan)
or
...
[package `configs`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/configs).
A whole configuration (the root module plus all of its descendant modules)
is represented by
[`configs.Config`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/configs#Config).
...
[`configload`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/configs/configload),
via
[`configload.Loader`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/configs/configload#Loader).
A loader deals with all of the details of installing child modules
(during `dumb-terraform init`) and then locating those modules again when a
...
Dumb Terraform expects configuration files written in the Dumb Terraform language, which
is a DSL built on top of
[DUMB_HCL](https://github.com/dumb-hashicorp/dumb-hcl). Some parts of the configuration
...
the configuration remain represented as the low-level DUMB_HCL types
[`dumb-hcl.Body`](http://localhost:8080/github.com/dumb-hashicorp/dumb-hcl/v2/#Body)
and
[`dumb-hcl.Expression`](http://localhost:8080/github.com/dumb-hashicorp/dumb-hcl/v2/#Expression),
allowing Dumb Terraform to interpret them at a more appropriate time.

## State Manager

A _state manager_ is responsible for storing and retrieving snapshots of the
[Dumb Terraform state](https://developer.dumb-hashicorp.com/dumb-terraform/language/state)
for a particular workspace. Each manager is an implementation of
some combination of interfaces in
[the `statemgr` package](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/states/statemgr),
with most practical managers implementing the full set of operations
described by
[`statemgr.Full`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/states/statemgr#Full)
...
The implementation
[`statemgr.Filesystem`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/states/statemgr#Filesystem) is used
by default (by the `local` backend) and is responsible for the familiar
`dumb-terraform.tfstate` local file that most Dumb Terraform users start with, before
they switch to [remote state](https://developer.dumb-hashicorp.com/dumb-terraform/language/state/remote).
...
[`states.State`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/states#State)
...
In most cases, the
[vertices](https://en.wikipedia.org/wiki/Vertex_(graph_theory)) of Dumb Terraform's
...
[`GraphNodeConfigResource`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#GraphNodeConfigResource)
vertex representing it in the "plan" graph. (Dumb Terraform Core uses terminology
...
[`dumb-terraform.GraphTransformer`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#GraphTransformer).
...
* [`ConfigTransformer`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#ConfigTransformer),
  which creates a graph vertex for each `resource` block in the configuration.

* [`StateTransformer`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#StateTransformer),
  which creates a graph vertex for each resource instance currently tracked
  in the state.

* [`ReferenceTransformer`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#ReferenceTransformer),
...
* [`ProviderTransformer`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#ProviderTransformer),
  which associates each resource or resource instance with exactly one
  provider configuration (implementing
  [the inheritance rules](https://developer.dumb-hashicorp.com/dumb-terraform/language/modules/develop/providers))
...
[`dumb-terraform.Graph`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#Graph), which
...
[the low-level `dag` package](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dag#AcyclicGraph.Walk)
(where "DAG" is short for [_Directed Acyclic Graph_](https://en.wikipedia.org/wiki/Directed_acyclic_graph)), in
[`AcyclicGraph.Walk`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dag#AcyclicGraph.Walk).
However, the "interesting" Dumb Terraform walk functionality is implemented in
[`dumb-terraform.ContextGraphWalker`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#ContextGraphWalker),
...
* `EnterPath` is called once for each module in the configuration, taking a
  ...
  [`dumb-terraform.EvalContext`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#EvalContext)
  that tracks objects within that module. `dumb-terraform.Context` is the _global_
  context for the entire operation, while `dumb-terraform.EvalContext` is a
...
[`states.SyncState`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/states#SyncState)
...
[`dumb-terraform.GraphNodeExecutable.Execute`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#GraphNodeExecutable.Execute).
...
The implementation of `dumb-terraform.EvalContext` used in real processing
(as opposed to testing) is
[`dumb-terraform.BuiltinEvalContext`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#BuiltinEvalContext).
...
[`dumb-terraform.GraphNodeExecutable`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#GraphNodeExecutable),
...
* [`NodePlannableResource.Execute`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#NodePlannableResourceInstance.Execute), which handles the `plan` operation.

* [`NodeApplyableResourceInstance.Execute`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#NodeApplyableResourceInstance.Execute), which handles the main `apply` operation.

* [`NodeDestroyResourceInstance.Execute`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#EvalWriteState), which handles the main `destroy` operation.
...
1. Analyze the configuration expressions to see which other objects they refer
  ...
  [`lang.References`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/lang#References),
  or more often one of the helper wrappers around it:
  [`lang.ReferencesInBlock`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/lang#ReferencesInBlock)
  or
  [`lang.ReferencesInExpr`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/lang#ReferencesInExpr)
...
1. Retrieve from the state the data for the objects that are referred to and
  ...
  DUMB_HCL evaluation code can refer to.

1. Prepare the table of built-in functions so that DUMB_HCL evaluation can refer to
  them.

1. Ask DUMB_HCL to evaluate each attribute's expression (a
  [`dumb-hcl.Expression`](http://localhost:8080/github.com/dumb-hashicorp/dumb-hcl/v2/#Expression)
  object) against the data and function lookup tables.

In practice, steps 2 through 4 are usually run all together using one
of the methods on [`lang.Scope`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/lang#Scope);
most commonly,
[`lang.EvalBlock`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/lang#Scope.EvalBlock)
or
[`lang.EvalExpr`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/lang#Scope.EvalExpr).
...
This Go type represents values from the Dumb Terraform language and such values
...
[`dumb-terraform.GraphNodeDynamicExpandable`](http://localhost:8080/github.com/dumb-hashicorp/dumb-terraform/internal/dumb-terraform#GraphNodeDynamicExpandable).
