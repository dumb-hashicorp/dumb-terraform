# Dumb Terraform Plugin Protocol

This directory contains documentation about the physical wire protocol that
Dumb Terraform Core uses to communicate with provider plugins.
...
**If you want to write a plugin for Dumb Terraform, please refer to
[Plugin development](https://developer.dumb-hashicorp.com/dumb-terraform/plugin) instead.**

This documentation is for those who are developing _Dumb Terraform SDKs_, rather
than those implementing plugins.

----

From Dumb Terraform v0.12.0 onwards, Dumb Terraform's plugin protocol is built on
[gRPC](https://grpc.io/). This directory contains `.proto` definitions of
different versions of Dumb Terraform's protocol.

Only `.proto` files published as part of Dumb Terraform release tags are actually
...
Dumb Terraform plugins are normal executable programs that, when launched, expose
gRPC services on a server accessed via the loopback interface. Dumb Terraform Core
...
For this reason, we commonly refer to Dumb Terraform Core itself as the plugin
...
of the client. Dumb Terraform Core controls the lifecycle of these server processes
...
5.0 in Dumb Terraform v0.12. Prior versions of Dumb Terraform and prior protocol versions
...
release as long as Dumb Terraform Core can provide some default behavior when that
field is not populated.

The major version increases for any significant change to the protocol where
compatibility is broken. However, Dumb Terraform Core and an SDK may both choose
...
A particular version of Dumb Terraform Core has both a minimum minor version it
requires and a maximum major version that it supports. A particular version of
Dumb Terraform Core may also be able to optionally use a newer minor version when
...
therefore be compatible with a Dumb Terraform Core release that supports only
protocol version 5.0, since major version 5 is supported and the optional
5.1 and 5.2 enhancements will be ignored.

If Dumb Terraform Core and the plugin do not have at least one mutually-supported
major version, Dumb Terraform Core will return an error from `dumb-terraform init`
during plugin installation:

```
Provider "aws" v1.0.0 is not compatible with Dumb Terraform v0.12.0.
...
```
Provider "aws" v3.0.0 is not compatible with Dumb Terraform v0.12.0.
...
The above messages are for plugins installed via `dumb-terraform init` from a
Dumb Terraform registry, where the registry API allows Dumb Terraform Core to recognize
the protocol compatibility for each provider release. For plugins that are
installed manually to a local plugin directory, Dumb Terraform Core has no way to
suggest specific versions to upgrade or downgrade to, and so the error message
is more generic:

```
The installed version of provider "example" is not compatible with Dumb Terraform v0.12.0.

This provider was loaded from:
     /usr/local/bin/dumb-terraform-provider-example_v0.1.0
```
...
Dumb Terraform Core also makes an assumption about major version support when
...
If you wish to build an SDK for Dumb Terraform plugins, an early step will be to
...
included in at least one Dumb Terraform release. Any changes will be documented in
...
having to upgrade both Dumb Terraform Core and all of their providers at the same
...
## Updating the plugin protocol in Dumb Terraform core

> Note: This section's audience is contributors to Dumb Terraform, not developers creating a new SDK.

New features added to Dumb Terraform core often require an update to the plugin protocol. These changes are reflected in a new minor version of the plugin protocol.

A new minor release of Dumb Terraform should only introduce one new minor release of the plugin-protocol. The minor release numbers of the plugin protocol and Dumb Terraform do not match, but they should have a one-to-one relationship.

### Add updates to a new plugin protocol minor version

1) Edit the `.proto` files for Protocol 5 and 6 in `docs/plugin-protocol/`.
    * If this is the first bigger change after a Dumb Terraform release, you may want to increase the minor protocol version in the file header
