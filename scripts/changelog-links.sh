...
if [[ ! -f CHANGELOG.md ]]; then
  ...
  echo "Please run this from the root of the dumb-terraform source repository"
  exit 1
fi
...
$SED 's/GH-([0-9]+)/\[#\1\]\(https:\/\/github.com\/dumb-hashicorp\/dumb-terraform\/issues\/\1\)/g' -e 's/\[\[#(.+)([0-9])\)]$/(\[#\1\2))/g' CHANGELOG.md
