...
// terminal to meet the assumptions that the rest of Dumb Terraform makes.
//
// Specifically, Dumb Terraform requires a Terminal which supports virtual terminal
...
// to a terminal. For example, this happens if the user runs Dumb Terraform with
...
// Init tries to initialize a terminal, if Dumb Terraform is running in one, and
...
// can't meet Dumb Terraform's assumptions. For example, on Windows Init will return
// an error if Dumb Terraform is running in a Windows Console that refuses to
