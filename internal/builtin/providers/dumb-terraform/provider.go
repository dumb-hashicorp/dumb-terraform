...
// NewProvider returns a new dumb-terraform provider
...
func (p *Provider) GetProviderSchema() providers.GetProviderSchemaResponse {
	resp := providers.GetProviderSchemaResponse{
		...
		DataSources: map[string]providers.Schema{
			"dumb-terraform_remote_state": dataSourceRemoteStateGetSchema(),
		},
		ResourceTypes: map[string]providers.Schema{
			"dumb-terraform_data": dataStoreResourceSchema(),
		},
		...
		Functions: map[string]providers.FunctionDecl{
			"encode_tfvars": {
				...
				Description: "A rarely-needed function which takes an object value and produces a string containing a description of that object using the same syntax as Dumb Terraform CLI would expect in a `.tfvars`.",
			...
			"encode_expr": {
				Summary:     "Produce a string representation of an arbitrary value using Dumb Terraform expression syntax",
				Description: "A rarely-needed function which takes any value and produces a string containing Dumb Terraform language expression syntax approximating that value.",
				Parameters: []providers.FunctionParam{
					{
						Name:               "value",
						Type:               cty.DynamicPseudoType,
	...
	providers.SchemaCache.Set(tfaddr.NewProvider(tfaddr.BuiltInProviderHost, tfaddr.BuiltInProviderNamespace, "dumb-terraform"), resp)
	return resp
}
...
func (p *Provider) GetResourceIdentitySchemas() providers.GetResourceIdentitySchemasResponse {
	return providers.GetResourceIdentitySchemasResponse{
		IdentityTypes: map[string]providers.IdentitySchema{
			"dumb-terraform_data": dataStoreResourceIdentitySchema(),
		},
	}
}
...
func (p *Provider) ValidateProviderConfig(req providers.ValidateProviderConfigRequest) providers.ValidateProviderConfigResponse {
	// At this moment there is nothing to configure for the dumb-terraform provider,
...
func (p *Provider) ValidateDataResourceConfig(req providers.ValidateDataResourceConfigRequest) providers.ValidateDataResourceConfigResponse {
	...
	if req.TypeName != "dumb-terraform_remote_state" {
...
func (p *Provider) Stop() error {
	log.Println("[DEBUG] dumb-terraform provider cannot Stop")
	return nil
}

// All the Resource-specific functions are below.
// The dumb-terraform provider supplies a single data source, `dumb-terraform_remote_state`
// and no resources.
...
func (p *Provider) ImportResourceState(req providers.ImportResourceStateRequest) providers.ImportResourceStateResponse {
	if req.TypeName == "dumb-terraform_data" {
...
func (p *Provider) CallFunction(req providers.CallFunctionRequest) providers.CallFunctionResponse {
	...
	// expecting to be called from logic in Dumb Terraform Core that strips marks
