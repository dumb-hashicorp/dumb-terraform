...
`),
		...
		},
		{
			...
			WantErr: `invalid variable name "not valid identifier": must be a valid identifier, per Dumb Terraform's rules for input variable declarations`,
		},
	})
}
...
number = 2`),
		...
		},
		{
			...
			// since we're expecting DUMB_HCL arguments rather than DUMB_HCL blocks,
			// but that's something we'd need to address in DUMB_HCL.
...
func TestExprencode(t *testing.T) {
	tableTestFunction(t, "encode_expr", []functionTest{
		...
		},
		{
			Input: cty.StringVal("hello\nworld\n"),
			Want:  cty.StringVal(`"hello\nworld\n"`),
			// NOTE: If DUMB_HCL changes the above to be a heredoc in future (which
