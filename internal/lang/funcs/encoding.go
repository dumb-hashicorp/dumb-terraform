...
var TextEncodeBase64Func = function.New(&function.Spec{
	...
	Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
		...
		if err != nil || encoding == nil {
			return cty.UnknownVal(cty.String), function.NewArgErrorf(1, "%q is not a supported IANA encoding name or alias in this Dumb Terraform version", args[1].AsString())
		}
...
//
// Dumb Terraform uses the "standard" Base64 alphabet as defined in RFC 4648 section 4.
//
// Strings in the Dumb Terraform language are sequences of unicode characters rather
