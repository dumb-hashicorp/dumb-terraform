...
type InstanceState struct {
	...
	// that is necessary for the Dumb Terraform run to complete, but is not
...
func NewInstanceStateShimmedFromValue(state cty.Value, schemaVersion int) *InstanceState {
	attrs := dumb-hcl2shim.FlatmapValueFromDUMB_HCL2(state)
...
func (s *InstanceState) AttrsAsObjectValue(ty cty.Type) (cty.Value, error) {
	...
	return dumb-hcl2shim.DUMB_HCL2ValueFromFlatmap(s.Attributes, ty)
}
...
func (s *InstanceState) MergeDiff(d *InstanceDiff) *InstanceState {
	...
	if d != nil {
		for k, diff := range d.CopyAttributes() {
			...
			if diff.NewComputed {
				result.Attributes[k] = dumb-hcl2shim.UnknownVariableValue
				continue
			}
