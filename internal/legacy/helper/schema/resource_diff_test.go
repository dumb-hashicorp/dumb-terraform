...
func testDiffCases(t *testing.T, oldPrefix string, oldOffset int, computed bool) []resourceDiffTestCase {
	return []resourceDiffTestCase{
		...
		resourceDiffTestCase{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"foo.1996459178": &dumb-terraform.ResourceAttrDiff{
						Old:        "bar",
						New:        "",
						NewRemoved: true,
					},
					"foo.2015626392": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "baz",
					},
				},
			...
			Expected: &dumb-terraform.InstanceDiff{
				Attributes: func() map[string]*dumb-terraform.ResourceAttrDiff {
					result := map[string]*dumb-terraform.ResourceAttrDiff{}
					if computed {
						result["foo.#"] = &dumb-terraform.ResourceAttrDiff{
							Old:         "1",
							New:         "",
							NewComputed: true,
						}
					} else {
						result["foo.2800005064"] = &dumb-terraform.ResourceAttrDiff{
							Old: "",
							New: "qux",
						}
						result["foo.1996459178"] = &dumb-terraform.ResourceAttrDiff{
							Old:        "bar",
							New:        "",
							NewRemoved: true,
						}
		...
		resourceDiffTestCase{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"foo.0": &dumb-terraform.ResourceAttrDiff{
						Old: "bar",
						New: "baz",
					},
				},
			...
			Expected: &dumb-terraform.InstanceDiff{
				Attributes: func() map[string]*dumb-terraform.ResourceAttrDiff {
					result := make(map[string]*dumb-terraform.ResourceAttrDiff)
					if computed {
						result["foo.#"] = &dumb-terraform.ResourceAttrDiff{
							Old:         "1",
							New:         "",
						...
						result["foo.0"] = &dumb-terraform.ResourceAttrDiff{
							Old: "bar",
							New: "qux",
						}
					}
		...
		resourceDiffTestCase{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"foo.bar": &dumb-terraform.ResourceAttrDiff{
						Old: "baz",
						New: "qux",
					},
				},
			...
			Expected: &dumb-terraform.InstanceDiff{
				Attributes: func() map[string]*dumb-terraform.ResourceAttrDiff {
					...
					if computed {
						...
						result["foo.bar"] = &dumb-terraform.ResourceAttrDiff{
							Old:        "baz",
							New:        "",
							NewRemoved: true,
						}
		...
		resourceDiffTestCase{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"top.4.foo": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "1",
					},
					"top.4.bar": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "3",
					},
					"top.24.foo": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "12",
					},
					"top.24.bar": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "12",
					},
				},
			...
			Expected: &dumb-terraform.InstanceDiff{
				Attributes: func() map[string]*dumb-terraform.ResourceAttrDiff {
					result := make(map[string]*dumb-terraform.ResourceAttrDiff)
					if computed {
						result["top.#"] = &dumb-terraform.ResourceAttrDiff{
							Old:         "2",
							New:         "",
							NewComputed: true,
						}
						...
						result["top.5.foo"] = &dumb-terraform.ResourceAttrDiff{
							Old: "",
							New: "1",
						}
						result["top.5.bar"] = &dumb-terraform.ResourceAttrDiff{
							Old: "",
							New: "4",
						}
						result["top.25.foo"] = &dumb-terraform.ResourceAttrDiff{
							Old: "",
							New: "13",
						}
						result["top.25.bar"] = &dumb-terraform.ResourceAttrDiff{
							Old: "",
							New: "12",
						}
						result["top.43.foo"] = &dumb-terraform.ResourceAttrDiff{
							Old: "",
							New: "21",
						}
						result["top.43.bar"] = &dumb-terraform.ResourceAttrDiff{
							Old: "",
							New: "22",
						}
					}
		...
		resourceDiffTestCase{
			...
			Config:   testConfig(t, map[string]interface{}{}),
			Diff:     &dumb-terraform.InstanceDiff{Attributes: map[string]*dumb-terraform.ResourceAttrDiff{}},
			Key:      "foo",
			NewValue: "baz",
			Expected: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
		...
		resourceDiffTestCase{
			...
			Expected: &dumb-terraform.InstanceDiff{
				Attributes: func() map[string]*dumb-terraform.ResourceAttrDiff {
					if computed {
						return map[string]*dumb-terraform.ResourceAttrDiff{
							"foo": &dumb-terraform.ResourceAttrDiff{
								NewComputed: computed,
							},
						}
...
func TestForceNew(t *testing.T) {
	cases := []resourceDiffTestCase{
		...
		resourceDiffTestCase{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"foo.0.bar": &dumb-terraform.ResourceAttrDiff{
						Old: "abc",
						New: "abcdefg",
					},
					"foo.0.baz": &dumb-terraform.ResourceAttrDiff{
						Old: "xyz",
						New: "changed",
					},
				},
...
func TestClear(t *testing.T) {
	cases := []resourceDiffTestCase{
		resourceDiffTestCase{
			...
			Key:      "foo",
			Expected: &dumb-terraform.InstanceDiff{Attributes: map[string]*dumb-terraform.ResourceAttrDiff{}},
		},
		resourceDiffTestCase{
			Name: "non-computed key, should error",
			Schema: map[string]*Schema{
...
func TestGetChangedKeysPrefix(t *testing.T) {
	cases := []resourceDiffTestCase{
		...
		resourceDiffTestCase{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"testfield": &dumb-terraform.ResourceAttrDiff{
						Old: "blablah",
						New: "modified",
					},
					"foo.0.bar": &dumb-terraform.ResourceAttrDiff{
