...
// ResourceImporter defines how a resource is imported in Dumb Terraform. This
...
//
// "Importing" in Dumb Terraform is the process of taking an already-created
// resource and bringing it under Dumb Terraform management. This can include
// updating Dumb Terraform state, generating Dumb Terraform configuration, etc.
type ResourceImporter struct {
	...
	// insert into the Dumb Terraform state. If this isn't specified, then
	// the ID is passed straight through.
	State StateFunc
}

// StateFunc is the function called to import a resource into the
// Dumb Terraform state. It is given a ResourceData with only ID set. This
