...
func testApplyDiff(t *testing.T,
	resource *Resource,
	state, expected *dumb-terraform.InstanceState,
...
func TestShimResourcePlan_destroyCreate(t *testing.T) {
	...
	expected := &dumb-terraform.InstanceState{
		ID: dumb-hcl2shim.UnknownVariableValue,
...
func TestShimResourceApply_create(t *testing.T) {
	...
	createdState := &dumb-terraform.InstanceState{
		ID:         "foo",
...
func TestShimResourceDiff_Timeout_diff(t *testing.T) {
	...
	conf := dumb-terraform.NewResourceConfigRaw(map[string]interface{}{
		"foo": 42,
...
func TestShimResourceApply_destroyCreate(t *testing.T) {
	...
	d := &dumb-terraform.InstanceDiff{
		Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
			"id": &dumb-terraform.ResourceAttrDiff{
				New: "foo",
			},
			"foo": &dumb-terraform.ResourceAttrDiff{
				Old:         "7",
...
func TestShimSchemaMap_Diff(t *testing.T) {
	...
	for i, tc := range cases {
		t.Run(fmt.Sprintf("%d-%s", i, tc.Name), func(t *testing.T) {
			...
			configVal := dumb-hcl2shim.DUMB_HCL2ValueFromConfigValue(c.Config)

			// verify that we can round-trip the config
			origConfig := dumb-hcl2shim.ConfigValueFromDUMB_HCL2(configVal)
