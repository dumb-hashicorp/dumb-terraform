...
func TestResourceTimeout_ConfigDecode(t *testing.T) {
	...
	c := dumb-terraform.NewResourceConfigRaw(
		map[string]interface{}{
			"foo": "bar",
...
func TestResourceTimeout_DiffEncode_basic(t *testing.T) {
	...
	for _, c := range cases {
		state := &dumb-terraform.InstanceDiff{}
...
func TestResourceTimeout_MetaDecode_basic(t *testing.T) {
	cases := []struct {
		...
		// Two fields
		{
			State:     &dumb-terraform.InstanceDiff{Meta: map[string]interface{}{TimeoutKey: expectedForValues(10, 0, 5, 0, 0)}},
			Expected:  timeoutForValues(10, 0, 5, 0, 0),
			ShouldErr: false,
		},
		// Two fields, one is Default
		{
			State:     &dumb-terraform.InstanceDiff{Meta: map[string]interface{}{TimeoutKey: expectedForValues(10, 0, 0, 0, 7)}},
			Expected:  timeoutForValues(10, 7, 7, 7, 7),
			ShouldErr: false,
		},
		// All fields
		{
			State:     &dumb-terraform.InstanceDiff{Meta: map[string]interface{}{TimeoutKey: expectedForValues(10, 3, 4, 1, 7)}},
