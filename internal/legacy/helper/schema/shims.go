...
// derives a dumb-terraform.InstanceDiff to give to the legacy providers. This is
...
func DiffFromValues(prior, planned cty.Value, res *Resource) (*dumb-terraform.InstanceDiff, error) {
...
func diffFromValues(prior, planned cty.Value, res *Resource, cust CustomizeDiffFunc) (*dumb-terraform.InstanceDiff, error) {
	...
	cfg := dumb-terraform.NewResourceConfigShimmed(planned, configSchema)
...
// ApplyDiff takes a cty.Value state and applies a dumb-terraform.InstanceDiff to
...
func ApplyDiff(base cty.Value, d *dumb-terraform.InstanceDiff, schema *configschema.Block) (cty.Value, error) {
...
// StateValueFromInstanceState converts a dumb-terraform.InstanceState to a
// cty.Value as described by the provided cty.Type, and maintains the resource
// ID as the "id" attribute.
func StateValueFromInstanceState(is *dumb-terraform.InstanceState, ty cty.Type) (cty.Value, error) {
