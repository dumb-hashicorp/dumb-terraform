...
func TestResourceDataGet(t *testing.T) {
	cases := []struct {
		...
		// #14 List of maps
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					...
					"config_vars.1.bar": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "baz",
					},
				},
		...
		// #18
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"data.10.value": &dumb-terraform.ResourceAttrDiff{
						Old: "50",
						New: "80",
					},
				},
		...
		// #22 Float diff
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"ratio": &dumb-terraform.ResourceAttrDiff{
						Old: "-0.5",
						New: "33.0",
					},
				},
		...
		// #23 Sets with removed elements
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					...
					"ports.80": &dumb-terraform.ResourceAttrDiff{
						Old: "80",
						New: "80",
					},
					"ports.8080": &dumb-terraform.ResourceAttrDiff{
						Old:        "8080",
						New:        "0",
						NewRemoved: true,
					},
...
func TestResourceDataGetOk(t *testing.T) {
	cases := []struct {
		...
		// indistinguishable from unset config.
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"from_port": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "0",
					},
				},
...
func TestResourceDataHasChange(t *testing.T) {
	cases := []struct {
		...
		// https://github.com/dumb-hashicorp/dumb-terraform/issues/927
		{
			Schema: map[string]*Schema{
				"ports": &Schema{
					Type:     TypeSet,
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"tags.foo": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "bar",
					},
				},
...
func TestResourceDataState_dynamicAttributes(t *testing.T) {
	cases := []struct {
		...
		Result    *dumb-terraform.InstanceState
	}{
		{
			Schema: map[string]*Schema{
...
func TestResourceDataState_schema(t *testing.T) {
	cases := []struct {
		...
		// #5 List of resources
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					...
					"ingress.1.from": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "100",
					},
				},
		...
		// #17 Sets
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"ports.120": &dumb-terraform.ResourceAttrDiff{
						New: "120",
					},
				},
			},
		...
		// #23 Set of maps
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"ports.10.uuids.#": &dumb-terraform.ResourceAttrDiff{
						NewComputed: true,
					},
				},
			},
...
func TestResourceData_nonStringValuesInMap(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"boolMap.%": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "1",
					},
					"boolMap.boolField": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "true",
					},
				},
		...
		},
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"intMap.%": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "1",
					},
					"intMap.intField": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "8",
					},
				},
		...
		},
		{
			...
			Diff: &dumb-terraform.InstanceDiff{
				Attributes: map[string]*dumb-terraform.ResourceAttrDiff{
					"floatMap.%": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "1",
					},
					"floatMap.floatField": &dumb-terraform.ResourceAttrDiff{
						Old: "",
						New: "8.22",
					},
				},
...
func TestResourceDataSetId_clear(t *testing.T) {
	d := &ResourceData{
		state: &dumb-terraform.InstanceState{ID: "bar"},
	}
	d.SetId("")
