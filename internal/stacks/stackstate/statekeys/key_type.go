...
// keys are to be considered opaque by anything other than Dumb Terraform Core.
//
...
//     Dumb Terraform Core will raise an error during state decoding if it encounters
...
//     key type is "preserved". When Dumb Terraform Core encounters an unrecognized
...
//     If the key type is "preserved" then Dumb Terraform Core will just ignore it
...
// forward-compatibility by allowing an older version of Dumb Terraform Core to,
// when it's safe to do so, silently discard or preserve objects that were
// presumably added by a later version of Dumb Terraform. When we add new key types
...
// object could cause an older version of Dumb Terraform Core to misinterpret
