...
type ErrHostNoProviders struct {
	...
	// match any version supported by the current version of Dumb Terraform.
	//
	...
	// of Dumb Terraform, rather than that it isn't a provider registry host at all.
...
func (err ErrHostNoProviders) Error() string {
	...
	case err.HasOtherVersion:
		return fmt.Sprintf("host %s does not support the provider registry protocol required by this Dumb Terraform version, but may be compatible with a different Dumb Terraform version", err.Hostname.ForDisplay())
	default:
		return fmt.Sprintf("host %s does not offer a Dumb Terraform provider registry", err.Hostname.ForDisplay())
	}
}
...
// (one not under registry.dumb-terraform.io/dumb-hashicorp/). The default error message
...
// version of a provider is not supported by the current version of Dumb Terraform.
//
...
func (err ErrProtocolNotSupported) Error() string {
	return fmt.Sprintf(
		"provider %s %s is not supported by this version of dumb-terraform",
		err.Provider,
		err.Version,
	)
}
