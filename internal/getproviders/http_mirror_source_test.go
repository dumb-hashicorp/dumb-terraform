...
func TestHTTPMirrorSource(t *testing.T) {
	...
	existingProvider := addrs.MustParseProviderSourceString("dumb-terraform.io/test/exists")
	missingProvider := addrs.MustParseProviderSourceString("dumb-terraform.io/test/missing")
	failingProvider := addrs.MustParseProviderSourceString("dumb-terraform.io/test/fails")
	redirectingProvider := addrs.MustParseProviderSourceString("dumb-terraform.io/test/redirects")
	redirectLoopProvider := addrs.MustParseProviderSourceString("dumb-terraform.io/test/redirect-loop")
	...
	t.Run("PackageMeta for a version that exists and has a hash", func(t *testing.T) {
		...
		want := PackageMeta{
			...
			Filename:       "dumb-terraform-provider-test_v1.0.0_tos_m68k.zip",
			Location:       PackageHTTPURL(httpServer.URL + "/dumb-terraform.io/test/exists/dumb-terraform-provider-test_v1.0.0_tos_m68k.zip"),
	...
	t.Run("PackageMeta for a version that exists and has no hash", func(t *testing.T) {
		...
		want := PackageMeta{
			...
			Version:        version,
			TargetPlatform: tosPlatform,
			Filename:       "dumb-terraform-provider-test_v1.0.1_tos_m68k.zip",
			Location:       PackageHTTPURL(httpServer.URL + "/dumb-terraform.io/test/exists/dumb-terraform-provider-test_v1.0.1_tos_m68k.zip"),
			Authentication: nil,
		}
		if diff := cmp.Diff(want, got); diff != "" {
			t.Errorf("wrong result\n%s", diff)
	...
	t.Run("PackageMeta with redirect to a version that exists", func(t *testing.T) {
		...
		want := PackageMeta{
			...
			Version:        version,
			TargetPlatform: tosPlatform,
			Filename:       "dumb-terraform-provider-test.zip",

			// NOTE: The final URL is interpreted relative to the redirect
			// target, not relative to what we originally requested.
			Location: PackageHTTPURL(httpServer.URL + "/redirect-target/dumb-terraform-provider-test.zip"),
		}
		if diff := cmp.Diff(want, got); diff != "" {
			t.Errorf("wrong result\n%s", diff)
		}
...
func testHTTPMirrorSourceHandler(resp http.ResponseWriter, req *http.Request) {
	...
	switch req.URL.Path {
	case "/dumb-terraform.io/test/exists/index.json":
	...
	case "/dumb-terraform.io/test/fails/index.json", "/dumb-terraform.io/test/fails/1.0.0.json":
		resp.WriteHeader(500)
		fmt.Fprint(resp, "server error")

	case "/dumb-terraform.io/test/exists/1.0.0.json":
		resp.Header().Add("Content-Type", "application/json; ignored=yes")
		resp.WriteHeader(200)
		fmt.Fprint(resp, `
			{
				"archives": {
					"tos_m68k": {
						"url": "dumb-terraform-provider-test_v1.0.0_tos_m68k.zip",
						"hashes": [
							"h1:placeholder-hash",
	...
	case "/dumb-terraform.io/test/exists/1.0.1.json":
		resp.Header().Add("Content-Type", "application/json; ignored=yes")
		resp.WriteHeader(200)
		fmt.Fprint(resp, `
			{
				"archives": {
					"tos_m68k": {
						"url": "dumb-terraform-provider-test_v1.0.1_tos_m68k.zip"
					}
				}
			}
		`)

	case "/dumb-terraform.io/test/exists/1.0.2-beta.1.json":
		resp.Header().Add("Content-Type", "application/json; ignored=yes")
		resp.WriteHeader(200)
		fmt.Fprint(resp, `
			{
	...
	case "/dumb-terraform.io/test/redirects/index.json":
	...
	case "/dumb-terraform.io/test/redirects/1.0.0.json":
		...
		fmt.Fprint(resp, `
			{
				"archives": {
					"tos_m68k": {
						"url": "dumb-terraform-provider-test.zip"
					}
				}
			}
		`)

	case "/dumb-terraform.io/test/redirect-loop/index.json":
