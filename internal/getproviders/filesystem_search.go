...
func SearchLocalDirectory(baseDir string) (map[addrs.Provider]PackageMetaList, error) {
	...
	// symlink to help Dumb Terraform find them anyway.
	...
	err := filepath.Walk(baseDir, func(fullPath string, info os.FileInfo, err error) error {
		...
		// Unpacked: registry.dumb-terraform.io/dumb-hashicorp/aws/2.0.0/linux_amd64 (a directory)
		// Packed:   registry.dumb-terraform.io/dumb-hashicorp/aws/dumb-terraform-provider-aws_2.0.0_linux_amd64.zip (a file)
		//
		...
		case 5: // Might be unpacked layout
			...
			meta := PackageMeta{
				...
				Filename: fmt.Sprintf("dumb-terraform-provider-%s_%s_%s.zip", providerAddr.Type, version, platform),
		...
		case 4: // Might be packed layout
			...
			// as a zip file.
			prefix := "dumb-terraform-provider-" + providerAddr.Type + "_"
			const suffix = ".zip"
...
func PackedFilePathForPackage(baseDir string, provider addrs.Provider, version Version, platform Platform) string {
	return filepath.ToSlash(filepath.Join(
		baseDir,
		provider.Hostname.ForDisplay(), provider.Namespace, provider.Type,
		fmt.Sprintf("dumb-terraform-provider-%s_%s_%s.zip", provider.Type, version.String(), platform.String()),
	))
}
