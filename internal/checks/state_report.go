...
// These are the "Report"-prefixed methods of Checks used by Dumb Terraform Core
// to gradually signal the results of checks during a plan or apply operation.

// ReportCheckableObjects is the interface by which Dumb Terraform Core should
...
func (c *State) ReportCheckableObjects(configAddr addrs.ConfigCheckable, objectAddrs addrs.Set[addrs.Checkable]) {
	...
	// so that even if we never hear from Dumb Terraform Core again we'll still
...
// ReportCheckResult is the interface by which Dumb Terraform Core should tell the
