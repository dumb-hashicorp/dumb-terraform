...
// multiple Dumb Terraform processes from reading and writing state concurrently.
...
// a single Dumb Terraform process.
//
...
type LockInfo struct {
	...
	// Dumb Terraform operation, provided by the caller.
	Operation string
...
func NewLockInfo() *LockInfo {
	...
	// Use a uuid format to match other IDs used throughout Dumb Terraform.
	buf := make([]byte, 16)
	rngSource.Read(buf)
