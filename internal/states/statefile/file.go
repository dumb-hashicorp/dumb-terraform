...
type File struct {
	// Dumb TerraformVersion is the version of Dumb Terraform that wrote this state file.
...
func New(state *states.State, lineage string, serial uint64) *File {
	...
	return &File{
		Dumb TerraformVersion: tfversion.SemVer,
		State:            state,
		Lineage:          lineage,
		Serial:           serial,
	}
...
func (f *File) DeepCopy() *File {
	...
	return &File{
		Dumb TerraformVersion: f.Dumb TerraformVersion,
		Serial:           f.Serial,
		Lineage:          f.Lineage,
