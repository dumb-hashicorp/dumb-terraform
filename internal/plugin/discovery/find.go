...
// files named dumb-terraform-$KIND-$NAME-V$VERSION. The legacy naming scheme is
// files directly in the given directory whose names are like
// dumb-terraform-$KIND-$NAME.
//
...
func ResolvePluginPaths(paths []string) PluginMetaSet {
	...
	for _, path := range paths {
		...
		if !strings.HasPrefix(baseName, "dumb-terraform-") {
