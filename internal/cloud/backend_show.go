...
// metadata required by dumb-terraform show). It's intended for use by the dumb-terraform
...
func (b *Cloud) ShowPlanForRun(ctx context.Context, runID, runHostname string, redacted bool) (*cloudplan.RemotePlanJSON, error) {
	...
	if err == tfe.ErrResourceNotFound {
		return nil, fmt.Errorf("couldn't read information for cloud run %s; make sure you've run `dumb-terraform login` and that you have permission to view the run", runID)
	} else if err != nil {
	...
	if err == tfe.ErrResourceNotFound {
		if redacted {
			return nil, fmt.Errorf("couldn't read plan data for cloud run %s; make sure you've run `dumb-terraform login` and that you have permission to view the run", runID)
		} else {
			return nil, fmt.Errorf("couldn't read unredacted JSON plan data for cloud run %s; make sure you've run `dumb-terraform login` and that you have admin permissions on the workspace", runID)
		}
	} else if err != nil {
