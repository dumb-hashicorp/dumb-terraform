module "child" {
  # NOTE: For this test we need a working absolute path so that Dumb Terraform
  ...
  # Dumb Terraform syntax and won't work outside of this test.
  #
  # Note that we're intentionally using the special // delimiter to
  # tell Dumb Terraform that it should treat the "package" directory as a
  # whole as a module package, with all of its descendants "downloaded"
  # (copied) together into ./.dumb-terraform/modules/child so that child
