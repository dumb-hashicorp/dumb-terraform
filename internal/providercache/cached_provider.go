...
// plugin.
//
// This is a bit messy and heuristic-y because historically Dumb Terraform used the
...
func (cp *CachedProvider) ExecutableFile() (string, error) {
	...
	// "dumb-terraform-provider-happycloud", followed by zero or more additional
	...
	// - dumb-terraform-provider-happycloud_v1.0.0
	// - dumb-terraform-provider-happycloud.exe
	//
	...
	// compatibility with Dumb Terraform 0.12.
	//
	...
	wantPrefix := "dumb-terraform-provider-" + cp.Provider.Type
