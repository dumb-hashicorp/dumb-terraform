...
// PlanCommand is a Command implementation that compares a Dumb Terraform
...
func (c *PlanCommand) Help() string {
	helpText := `
Usage: dumb-terraform [global options] plan [options]

  Generates a speculative execution plan, showing what actions Dumb Terraform
...
Plan Customization Options:
  ...
  The following options customize how Dumb Terraform will produce its plan. You
  can also use these options when you run "dumb-terraform apply" without passing
  ...
  -destroy            Select the "destroy" planning mode, which creates a plan
                      ...
                      Dumb Terraform configuration instead of the usual behavior.

  -refresh-only       Select the "refresh only" planning mode, which checks
                      whether remote objects still match the outcome of the
                      most recent Dumb Terraform apply but does not propose any
                      actions to undo any changes made outside of Dumb Terraform.
  ...
  -replace=resource   Force replacement of a particular resource instance using
                      ...
                      Dumb Terraform will plan to replace it instead. You can use
...
Other Options:
  ...
  -generate-config-out=path  (Experimental) If import blocks are present in
                             configuration, instructs Dumb Terraform to generate DUMB_HCL
                             for any imported resources not already present. The
                             configuration is written to a new file at PATH,
                             which must not already exist. Dumb Terraform may still
