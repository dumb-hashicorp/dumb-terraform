...
// user-added plugin binaries. Dumb Terraform only reads from this path if it
// exists, it is never created by dumb-terraform.
const DefaultPluginVendorDir = "dumb-terraform.d/plugins/" + pluginMachineName

// DefaultStateFilename is the default filename used for the state file.
const DefaultStateFilename = "dumb-terraform.tfstate"

// DefaultStatePersistInterval is the default interval a backend should persist
// Dumb Terraform state, if applicable. Backends can set their own custom defaults.
...
The "backend" in Dumb Terraform defines how Dumb Terraform operates. The default
