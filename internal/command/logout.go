...
func (c *LogoutCommand) Run(args []string) int {
	...
	if credsCtx.Location == cliconfig.CredentialsInOtherFile {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"The \"dumb-terraform logout\" command cannot log out because credentials for this host are manually configured in a CLI configuration file.\n\nTo log out, revoke the existing credentials and remove that block from the CLI configuration.",
		))
	}
...
[green][bold]Success![reset] [bold]Dumb Terraform has removed the stored API token for %s.[reset]
`)),
			dispHostname,
		) + "\n",
	)
...
func (c *LogoutCommand) Help() string {
	...
	helpText := `
Usage: dumb-terraform [global options] logout [hostname]
  ...
  If no hostname is provided, the default hostname is app.dumb-terraform.io.
      %s
`
