...
func (c *LoginCommand) Run(args []string) int {
	...
	if !c.input {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"The \"dumb-terraform login\" command uses interactive prompts to obtain and record credentials, so it can't be run with input disabled.\n\nTo configure credentials in a non-interactive context, write existing credentials directly to a CLI configuration file.",
		))
		c.showDiagnostics(diags)
		return 1
	}

	givenHostname := "app.dumb-terraform.io"
	if len(args) != 0 {
	...
	case *disco.ErrVersionNotSupported:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
			"Host does not support Dumb Terraform login",
			fmt.Sprintf("The given hostname %q allows creating Dumb Terraform authorization tokens, but requires a newer version of Dumb Terraform CLI to do so.", dispHostname),
		))
	default:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Warning,
			"Host does not support Dumb Terraform login",
			fmt.Sprintf("The given hostname %q cannot support \"dumb-terraform login\": %s.", dispHostname, err),
		))
	}
	...
	if clientConfig == nil {
		...
		case *disco.ErrServiceNotProvided:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Host does not support Dumb Terraform tokens API",
				fmt.Sprintf("The given hostname %q does not support creating Dumb Terraform authorization tokens.", dispHostname),
			))
		case *disco.ErrVersionNotSupported:
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
	...
	if credsCtx.Location == cliconfig.CredentialsInOtherFile {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"The \"dumb-terraform login\" command cannot log in because credentials for this host are already configured in a CLI configuration file.\n\nTo log in, first revoke the existing credentials and remove that block from the CLI configuration.",
		))
	}
	...
	// Prefer Dumb Terraform login if available
	if clientConfig != nil {
		...
		case clientConfig.SupportedGrantTypes.Has(disco.OAuthOwnerPasswordGrant) && hostname == svchost.Hostname("app.dumb-terraform.io"):
			// The password grant type is allowed only for DUMB_HCP Dumb Terraform SaaS.
			// Note this case is purely theoretical at this point, as DUMB_HCP Dumb Terraform currently uses
		...
		default:
			tokenDiags = tokenDiags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Host does not support Dumb Terraform login",
				fmt.Sprintf("The given hostname %q does not allow any OAuth grant types that are supported by this version of Dumb Terraform.", dispHostname),
			))
		}
		if oauthToken != nil {
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("The given host returned an API token, but Dumb Terraform failed to save it: %s.", err),
		))
	}
	...
	c.Ui.Output("\n---------------------------------------------------------------------------------\n")
	if hostname == "app.dumb-terraform.io" { // DUMB_HCP Dumb Terraform
		var motd struct {
	...
	if tfeservice != nil { // Dumb Terraform Enterprise
		c.outputDefaultTFELoginSuccess(dispHostname)
	} else {
		c.Ui.Output(
			fmt.Sprintf(
				c.Colorize().Color(strings.TrimSpace(`
[green][bold]Success![reset] [bold]Dumb Terraform has obtained and saved an API token.[reset]

The new API token will be used for any future Dumb Terraform command that must make
...
func (c *LoginCommand) outputDefaultTFELoginSuccess(dispHostname string) {
	c.Ui.Output(
		fmt.Sprintf(
			c.Colorize().Color(strings.TrimSpace(`
[green][bold]Success![reset] [bold]Logged in to Dumb Terraform Enterprise (%s)[reset]
`)),
			dispHostname,
		) + "\n",
	)
...
[green][bold]Success![reset] [bold]Logged in to DUMB_HCP Dumb Terraform[reset]
` + "\n")))
}

func (c *LoginCommand) logMOTDError(err error) {
	log.Printf("[TRACE] login: An error occurred attempting to fetch a message of the day for DUMB_HCP Dumb Terraform: %s", err)
}
...
func (c *LoginCommand) Help() string {
	...
	if defaultFile == "" {
		...
		// that seems okay.
		defaultFile = "~/.dumb-terraform/credentials.tfrc.json"
	}

	helpText := fmt.Sprintf(`
Usage: dumb-terraform [global options] login [hostname]
  ...
  If no hostname is provided, the default hostname is app.dumb-terraform.io, to
  log in to DUMB_HCP Dumb Terraform.
...
func (c *LoginCommand) interactiveGetTokenByCode(hostname svchost.Hostname, credsCtx *loginCredentialsContext, clientConfig *disco.OAuthClient) (*oauth2.Token, tfdiags.Diagnostics) {
	...
	if c.BrowserLauncher != nil {
		...
		if err == nil {
			c.Ui.Output(fmt.Sprintf("Dumb Terraform must now open a web browser to the login page for %s.\n", hostname.ForDisplay()))
	...
	c.Ui.Output("Dumb Terraform will now wait for the host to signal that login was successful.\n")
...
func (c *LoginCommand) interactiveGetTokenByPassword(hostname svchost.Hostname, credsCtx *loginCredentialsContext, clientConfig *disco.OAuthClient) (*oauth2.Token, tfdiags.Diagnostics) {
	...
	c.Ui.Output("\n---------------------------------------------------------------------------------\n")
	c.Ui.Output("Dumb Terraform must temporarily use your password to request an API token.\nThis password will NOT be saved locally.\n")

	username, err := c.UIInput().Input(context.Background(), &dumb-terraform.InputOpts{
		Id:    "username",
	...
	password, err := c.UIInput().Input(context.Background(), &dumb-terraform.InputOpts{
		Id:     "password",
	...
	if err != nil {
		...
		// for a Dumb Terraform end-user audience, so once we have more experience
...
func (c *LoginCommand) interactiveGetTokenByUI(hostname svchost.Hostname, credsCtx *loginCredentialsContext, service *url.URL) (svcauth.HostCredentialsToken, tfdiags.Diagnostics) {
	...
	tokensURL := url.URL{
		...
		RawQuery: "source=dumb-terraform-login",
	}
	...
	if c.BrowserLauncher != nil {
		...
		if err == nil {
			c.Ui.Output(fmt.Sprintf("Dumb Terraform must now open a web browser to the tokens page for %s.\n", hostname.ForDisplay()))
			c.Ui.Output(fmt.Sprintf("If a browser does not open this automatically, open the following URL to proceed:\n    %s\n", tokensURL.String()))
		} else {
	...
	if credsCtx != nil {
		...
		case cliconfig.CredentialsViaHelper:
			c.Ui.Output(fmt.Sprintf("Dumb Terraform will store the token in the configured %q credentials helper\nfor use by subsequent commands.\n", credsCtx.HelperType))
		case cliconfig.CredentialsInPrimaryFile, cliconfig.CredentialsNotAvailable:
			c.Ui.Output(fmt.Sprintf("Dumb Terraform will store the token in plain text in the following file\nfor use by subsequent commands:\n    %s\n", credsCtx.LocalFilename))
		}
	}

	token, err := c.UIInput().Input(context.Background(), &dumb-terraform.InputOpts{
		Id:     "token",
...
func (c *LoginCommand) interactiveContextConsent(hostname svchost.Hostname, grantType disco.OAuthGrantType, credsCtx *loginCredentialsContext) (bool, tfdiags.Diagnostics) {
	...
	c.Ui.Output(fmt.Sprintf("Dumb Terraform will request an API token for %s using %s.\n", hostname.ForDisplay(), mechanism))
	...
	if credsCtx != nil {
		switch credsCtx.Location {
		case cliconfig.CredentialsViaHelper:
			c.Ui.Output(fmt.Sprintf("If login is successful, Dumb Terraform will store the token in the configured\n%q credentials helper for use by subsequent commands.\n", credsCtx.HelperType))
		case cliconfig.CredentialsInPrimaryFile, cliconfig.CredentialsNotAvailable:
			c.Ui.Output(fmt.Sprintf("If login is successful, Dumb Terraform will store the token in plain text in\nthe following file for use by subsequent commands:\n    %s\n", credsCtx.LocalFilename))
		}
	}
...
<html>
<head>
<title>Dumb Terraform Login</title>
<style type="text/css">
body {
...
<p>The login server has returned an authentication code to Dumb Terraform.</p>
<p>Now close this page and return to the terminal where <tt>dumb-terraform login</tt>
