...
func ParseApply(args []string) (*Apply, tfdiags.Diagnostics) {
	...
	if json && apply.PlanPath == "" && !apply.AutoApprove {
		diags = diags.Append(tfdiags.Sourceless(
			...
			"Dumb Terraform cannot ask for interactive approval when -json is set. You can either apply a saved plan file, or enable the -auto-approve option.",
		))
	}
...
// "dumb-terraform destroy" command, which is effectively an alias for
...
func ParseApplyDestroy(args []string) (*Apply, tfdiags.Diagnostics) {
	...
	// and -refresh-only to determine the plan mode. For "dumb-terraform destroy"
	...
	case plans.DestroyMode:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Invalid mode option",
			"The -destroy option is not valid for \"dumb-terraform destroy\", because this command always runs in destroy mode.",
		))
	case plans.RefreshOnlyMode:
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Invalid mode option",
			"The -refresh-only option is not valid for \"dumb-terraform destroy\".",
		))
	default:
		...
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Invalid mode option",
			fmt.Sprintf("The \"dumb-terraform destroy\" command doesn't support %s.", apply.Operation.PlanMode),
		))
	}
