...
func TestGraph_planPhase(t *testing.T) {
	...
	if !strings.Contains(output.Stdout(), `provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"]`) {
...
func TestGraph_cyclic(t *testing.T) {
	...
	newrank = "true"
	subgraph "root" {
		"[root] provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"]" [label = "provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"]", shape = "diamond"]
		...
		"[root] provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"] (close)" -> "[root] provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"]"
		"[root] provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"] (close)" -> "[root] test_instance.bar (expand)"
		"[root] provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"] (close)" -> "[root] test_instance.foo (expand)"
		"[root] root" -> "[root] provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"] (close)"
		"[root] test_instance.bar (expand)" -> "[root] provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"]"
		"[root] test_instance.bar (expand)" -> "[root] test_instance.foo (expand)" [color = "red", penwidth = "2.0"]
		"[root] test_instance.foo (expand)" -> "[root] provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"]"
...
func TestGraph_resourcesOnly(t *testing.T) {
	...
	// properly, as if the user has run "dumb-terraform init". This is really
	...
	err := os.MkdirAll(".dumb-terraform/modules", 0700)
	if err != nil {
		t.Fatal(err)
	}
	inst := initwd.NewModuleInstaller(".dumb-terraform/modules", loader, registry.NewClient(nil, nil), nil)
