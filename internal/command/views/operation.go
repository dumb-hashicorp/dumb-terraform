...
type Operation interface {
	...
	Plan(plan *plans.Plan, schemas *dumb-terraform.Schemas)
...
func (v *OperationHuman) Plan(plan *plans.Plan, schemas *dumb-terraform.Schemas) {
...
// the plan.
func (v *OperationJSON) Plan(plan *plans.Plan, schemas *dumb-terraform.Schemas) {
...
Note: You didn't use the -out option to save this plan, so Dumb Terraform can't guarantee to take exactly these actions if you run "dumb-terraform apply" now.
`
...
To perform exactly these actions, run the following command to apply:
    dumb-terraform apply %q
`

const planHeaderGenConfig = `
Dumb Terraform has generated configuration and written it to %s. Please review the configuration and edit it as necessary before adding it to version control.
`
