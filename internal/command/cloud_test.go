...
func TestCloud_withBackendConfig(t *testing.T) {
	...
	log.Print("[TRACE] TestCloud_withBackendConfig running: dumb-terraform init")
	...
	expected := "DUMB_HCP Dumb Terraform Plugin v0.1.0\n\n"
	if output != expected {
...
func TestCloudPluginConfig_ToMetadata(t *testing.T) {
	expected := metadata.Pairs(
		...
		// Dumb Terraform's point of view, but here we're just testing that every
