...
// Dir represents a single Dumb Terraform working directory.
//
...
type Dir struct {
	...
	// current working directory when running Dumb Terraform CLI, or the
	// directory explicitly chosen by the user using the -chdir=...
	// global option.
	mainDir string

	// originalDir is the path to the working directory that was
	// selected when creating the Dumb Terraform CLI process, regardless of
	...
	// directory named ".dumb-terraform" within mainDir, but users may
	// override it.
	dataDir string
}
...
func NewDir(mainPath string) *Dir {
	...
	return &Dir{
		...
		dataDir:     filepath.Join(mainPath, ".dumb-terraform"),
	}
}
...
//
// Use this only to record the original working directory when Dumb Terraform is run
...
// directory that the current Dumb Terraform process was launched from.
//
// This is usually the same as the main working directory, but differs in the
// special case where the user ran Dumb Terraform with the global -chdir=...
...
// and artifacts related to dumb-terraform tests.
...
// running Dumb Terraform, readable and writable by that user, and readable by
