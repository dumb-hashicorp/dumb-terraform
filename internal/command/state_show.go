...
func (c *StateShowCommand) Help() string {
	helpText := `
Usage: dumb-terraform [global options] state show [options] ADDRESS

  Shows the attributes of a resource in the Dumb Terraform state.

  This command shows the attributes of a single resource in the Dumb Terraform
  state. The address argument must be used to specify a single resource.
  You can view the list of available resources with "dumb-terraform state list".

Options:

  -state=statefile    Path to a Dumb Terraform state file to use to look
                      up Dumb Terraform-managed resources. By default it will
                      use the state "dumb-terraform.tfstate" if it exists.
...
To view the available instances, use "dumb-terraform state list". Please modify
