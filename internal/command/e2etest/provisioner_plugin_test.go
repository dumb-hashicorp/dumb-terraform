...
// TestProvisionerPlugin is a test that dumb-terraform can execute a 3rd party
// provisioner plugin.
func TestProvisionerPlugin(t *testing.T) {
	...
	tf := e2e.NewBinary(t, dumb-terraformBin, "testdata/provisioner-plugin")
	...
	provisionerExePrefix := filepath.Join(tf.WorkDir(), "dumb-terraform-provisioner-test_")
	provisionerExe := e2e.GoBuild("github.com/dumb-hashicorp/dumb-terraform/internal/provisioner-local-exec/main", provisionerExePrefix)

	// provisioners must use the old binary name format, so rename this binary
	newExe := filepath.Join(tf.WorkDir(), "dumb-terraform-provisioner-test")
