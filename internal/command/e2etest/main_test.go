...
// them, as we do for dumb-terraformBin.
...
func setup() func() {
	if dumb-terraformBin != "" {
		...
		var err error
		dumb-terraformBin, err = filepath.Abs(dumb-terraformBin)
		if err != nil {
			panic(fmt.Sprintf("failed to find absolute path of dumb-terraform executable: %s", err))
		}
		return func() {}
	}

	tmpFilename := e2e.GoBuild("github.com/dumb-hashicorp/dumb-terraform", "dumb-terraform")

	// Make the executable available for use in tests
	dumb-terraformBin = tmpFilename
