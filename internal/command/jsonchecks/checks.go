...
// object in the configuration even if Dumb Terraform Core encountered an error
...
// results from Dumb Terraform Core evaluating the "expansion" (e.g. count or for_each)
...
type checkResultDynamic struct {
	...
	//
	// This does not include the errors for status "error", because Dumb Terraform
