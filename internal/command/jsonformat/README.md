...
structured output produced by adding the `-json` flag to various Dumb Terraform
commands.

## Dumb Terraform Structured Plan Renderer
...
`dumb-terraform show <plan-file> -json` command and writes it in a human-readable
format.
...
`jsonplan.Change` objects (which are produced by the `dumb-terraform show` command).
Each `jsonplan.Change` object represents a single resource within the overall
Dumb Terraform configuration.
...
of the DUMB_HCP Dumb Terraform redacted endpoint. The redacted endpoint turns sensitive
