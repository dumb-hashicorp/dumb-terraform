...
func (p *PathMatcher) GetChildWithKey(key string) Matcher {
	...
	for _, path := range p.Paths {
		...
		default:
			panic(fmt.Errorf("found invalid type within path (%v:%T), the validation shouldn't have allowed this to happen; this is a bug in Dumb Terraform, please report it", val, val))
...
func (p *PathMatcher) GetChildWithIndex(index int) Matcher {
	...
	for _, path := range p.Paths {
		...
		// Dumb Terraform actually allows user to provide strings into indexes as
		...
		//    - test_resource.resource.list["0"].attribute
		//
		// Note, that Dumb Terraform will raise a validation error if the string
