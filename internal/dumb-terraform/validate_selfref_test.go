...
func TestValidateSelfRef(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			rAddr,
			dumb-hcltest.MockExprLiteral(cty.StringVal("bar")),
			false,
		},

		{
			"non self reference",
			rAddr,
			dumb-hcltest.MockExprTraversalSrc("aws_instance.bar.id"),
			false,
		},

		{
			"self reference",
			rAddr,
			dumb-hcltest.MockExprTraversalSrc("aws_instance.foo.id"),
			true,
		},

		{
			...
			rAddr,
			dumb-hcltest.MockExprTraversalSrc("aws_instance.foo[4].id"),
			false,
		},
...

		{
			...
			rAddr.Instance(addrs.IntKey(4)),
			dumb-hcltest.MockExprTraversalSrc("aws_instance.foo"),
			true,
		},
	}
