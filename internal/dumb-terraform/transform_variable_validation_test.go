		...
		rules: []*configs.CheckRule{
			{
				...
				Condition:    dumb-hcltest.MockExprTraversalSrc("var.foo"),
				ErrorMessage: dumb-hcltest.MockExprLiteral(cty.StringVal("wrong")),
			},
		},
	}
		...
		rules: []*configs.CheckRule{
			{
				...
				ErrorMessage: dumb-hcltest.MockExprTraversalSrc("var.foo"),
			},
		},
	}
	g.Add(fooNode)
...
func (n *nodeTestOnlyInputVariable) variableValidationRules() (addrs.ConfigInputVariable, []*configs.CheckRule, dumb-hcl.Range) {
	return n.configAddr, n.rules, dumb-hcl.Range{
		Filename: "test",
