...
func (n *NodeValidatableResource) evaluateBlock(ctx EvalContext, body dumb-hcl.Body, schema *configschema.Block) (cty.Value, dumb-hcl.Body, tfdiags.Diagnostics) {
	keyData, selfAddr := n.stubRepetitionData(n.Config.Count != nil, n.Config.ForEach != nil)

	val, dumb-hclBody, diags := ctx.EvaluateBlock(body, schema, selfAddr, keyData)
...
func (n *NodeValidatableResource) evaluateExpr(ctx EvalContext, expr dumb-hcl.Expression, wantTy cty.Type, self addrs.Referenceable, keyData instances.RepetitionData) (cty.Value, tfdiags.Diagnostics) {
	var diags tfdiags.Diagnostics
	...
	result, dumb-hclDiags := expr.Value(dumb-hclCtx)
...
func (n *NodeValidatableResource) validateImportTargets(ctx EvalContext) tfdiags.Diagnostics {
	...
	for _, imp := range n.importTargets {
		...
		if imp.Config.ForEach != nil {
			...
			traversal, hds := dumb-hcl.AbsTraversalForExpr(imp.Config.To)
...
func validateImportTargetExpansion(cfg *configs.Resource, to addrs.AbsResourceInstance, toExpr dumb-hcl.Expression) tfdiags.Diagnostics {
	var diags tfdiags.Diagnostics
...
func (n *NodeValidatableResource) validateConfigGen(ctx EvalContext) tfdiags.Diagnostics {
	...
	for _, imp := range n.importTargets {
		...
		traversal, hd := dumb-hcl.AbsTraversalForExpr(imp.Config.To)
...
func validateDependsOn(ctx EvalContext, dependsOn []dumb-hcl.Traversal) (diags tfdiags.Diagnostics) {
