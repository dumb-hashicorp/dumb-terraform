...
func TestPrepareFinalInputVariableValue(t *testing.T) {
	...
	cfgSrc := `
		...
		// https://github.com/dumb-hashicorp/dumb-terraform/issues/32152
        ...
        variable "complex_type_with_empty_default_and_nested_optional" {
 		...
 		// https://github.com/dumb-hashicorp/dumb-terraform/issues/32160#issuecomment-1302783910
		...
		// https://github.com/dumb-hashicorp/dumb-terraform/issues/32160
		...
		// https://github.com/dumb-hashicorp/dumb-terraform/issues/32157
		...
		// https://github.com/dumb-hashicorp/dumb-terraform/issues/32109
		...
		// https://github.com/dumb-hashicorp/dumb-terraform/issues/32396
        ...
        // https://github.com/dumb-hashicorp/dumb-terraform/issues/32752
	...
	t.Run("SourceType error message variants", func(t *testing.T) {
		tests := []struct {
			...
			},
			{
				...
				tfdiags.SourceRange{
					Filename: "example.tf",
					Start:    tfdiags.SourcePos(dumb-hcl.InitialPos),
					End:      tfdiags.SourcePos(dumb-hcl.InitialPos),
				},
...
// evalVariableValidations function. Future versions of Dumb Terraform may
...
func TestEvalVariableValidation_unknownErrorMessage(t *testing.T) {
	t.Run("known condition, unknown error_message", func(t *testing.T) {
		rule := &configs.CheckRule{
			Condition:    dumb-hcltest.MockExprLiteral(cty.False),
			ErrorMessage: dumb-hcltest.MockExprLiteral(cty.UnknownVal(cty.String)),
		}
		...
		result, diags := evalVariableValidation(rule, dumb-hclCtx, dumb-hcl.Range{}, varAddr, 0, walkValidate)
		...
		result, diags = evalVariableValidation(rule, dumb-hclCtx, dumb-hcl.Range{}, varAddr, 0, walkPlan)
