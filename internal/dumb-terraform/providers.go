...
func checkExternalProviders(rootCfg *configs.Config, plan *plans.Plan, state *states.State, got map[addrs.RootProviderConfig]providers.Interface) tfdiags.Diagnostics {
	...
	// we can't be precise because Dumb Terraform permits implicit default provider
	// configurations.)
...
// and pre-configured.
//
// This is a kinda-hacky way to deal with the fact that Dumb Terraform Core
...
// be pre-configured before passing them to Dumb Terraform Core.
