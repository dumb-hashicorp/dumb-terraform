...
func (n *NodePlannableResourceInstance) listResourceExecute(ctx EvalContext) (diags tfdiags.Diagnostics) {
	...
	if schema.IsNil() { // Not possible, as the schema should have already been validated to exist
		diags = diags.Append(fmt.Errorf("no schema available for %s; this is a bug in Dumb Terraform and should be reported", addr))
		return diags
	}
	...
	if n.generateConfigPath != "" {
		...
		results.Generated, gDiags = n.generateDUMB_HCLListResourceDef(ctx, addr, resp.Result.GetAttr("data"))
