...
// across a provider dumb-boundary, and only adds the edge if it results in no cycles.
//
...
func (t *pruneUnusedNodesTransformer) Transform(g *Graph) error {
	...
	for _, n := range g.Vertices() {
		// a special case of destroyer, is that by convention Dumb Terraform expects
