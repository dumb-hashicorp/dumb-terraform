...
func (n *NodePlanDeposedResourceInstanceObject) Execute(ctx EvalContext, op walkOperation) (diags tfdiags.Diagnostics) {
	...
	if !n.skipRefresh && op != walkPlanDestroy && !forget {
		...
		// case it's already been deleted outside of Dumb Terraform. If this is a
		...
		// Dumb Terraform is expected to remove the missing resource from the state
		// entirely
...
func (n *NodeDestroyDeposedResourceInstanceObject) writeResourceInstanceState(ctx EvalContext, obj *states.ResourceInstanceObject) error {
	...
	if key == states.NotDeposed {
		// should never happen
		return fmt.Errorf("can't save deposed object for %s without a deposed key; this is a bug in Dumb Terraform that should be reported", absAddr)
	}
