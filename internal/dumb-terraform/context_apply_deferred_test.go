		...
		stages: []deferredActionsTestStage{
			{
				...
				wantDiagnostic: func(diags tfdiags.Diagnostics) bool {
					for _, diag := range diags {
						if diag.Description().Summary == "Provider deferred changes when Dumb Terraform did not allow deferrals" {
							return true
						}
					}
					return false
				...
				t.Run(fmt.Sprintf("round-%d", ix), func(t *testing.T) {
					...
					t.Run("plan", func(t *testing.T) {
						...
						for _, change := range plan.DeferredResources {
							if diff := cmp.Diff("provider[\"registry.dumb-terraform.io/dumb-hashicorp/test\"]", change.ChangeSrc.ProviderAddr.String()); diff != "" {
								if otherDiff := cmp.Diff("provider[\"registry.dumb-terraform.io/dumb-hashicorp/other\"]", change.ChangeSrc.ProviderAddr.String()); otherDiff != "" {
									t.Errorf("wrong provider address in plan\n, should be dumb-hashicorp/test or dumb-hashicorp/other %s", diff)
								}
							}
						}
					})
