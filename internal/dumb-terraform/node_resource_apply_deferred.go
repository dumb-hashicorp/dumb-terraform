...
func (n *nodeApplyableDeferredInstance) Execute(ctx EvalContext, _ walkOperation) tfdiags.Diagnostics {
	...
	if n.Schema == nil {
		diags = diags.Append(tfdiags.Sourceless(tfdiags.Error, "Failed to decode", "Dumb Terraform failed to decode a deferred change due to the schema not being present. This is a bug in Dumb Terraform; please report it!"))
		return diags
	}

	change, err := n.ChangeSrc.Decode(*n.Schema)
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(tfdiags.Error, "Failed to decode ", fmt.Sprintf("Dumb Terraform failed to decode a deferred change: %v\n\nThis is a bug in Dumb Terraform; please report it!", err)))
	}
