...
func TestContextEval(t *testing.T) {
	...
	// Since we're testing 'eval' (used by dumb-terraform console), impure functions
	...
	for _, test := range tests {
		t.Run(test.Input, func(t *testing.T) {
			...
			expr, _ := dumb-hclsyntax.ParseExpression([]byte(test.Input), "<test-input>", dumb-hcl.Pos{Line: 1, Column: 1})
...
func TestContextPlanAndEval(t *testing.T) {
	...
	t.Run("var.a", func(t *testing.T) {
		expr := dumb-hcltest.MockExprTraversalSrc(`var.a`)
	...
	t.Run("test_thing.a", func(t *testing.T) {
		expr := dumb-hcltest.MockExprTraversalSrc(`test_thing.a`)
