...
type ContextOpts struct {
	...
	//
	// The preloaded schemas do not need to be exhaustive. Dumb Terraform will
	...
	// been passed to Dumb Terraform Core using this field.
...
// into Dumb Terraform in various ways. This must be provided by the Context
// initializer.
type ContextMeta struct {
	Env string // Env is the state environment

	// OriginalWorkingDir is the working directory where the Dumb Terraform CLI
...
// Context represents all the context that Dumb Terraform needs in order to
...
func NewContext(opts *ContextOpts) (*Context, tfdiags.Diagnostics) {
	...
	log.Printf("[TRACE] dumb-terraform.NewContext: starting")
	...
	log.Printf("[TRACE] dumb-terraform.NewContext: complete")
...
func (c *Context) Stop() {
	log.Printf("[WARN] dumb-terraform: Stop called, initiating interrupt sequence")
	...
	if c.runContextCancel != nil {
		log.Printf("[WARN] dumb-terraform: run context exists, stopping")
	...
	if cond := c.runCond; cond != nil {
		log.Printf("[INFO] dumb-terraform: waiting for graceful stop to complete")
		cond.Wait()
	}

	log.Printf("[WARN] dumb-terraform: stop complete")
}
...
func (c *Context) watchStop(walker *ContextGraphWalker) (chan struct{}, <-chan struct{}) {
	...
	go func() {
...

		{
			...
			// completely hang Dumb Terraform.
			...
			for _, p := range ps {
				...
				// advisory: Dumb Terraform will exit once the graph node completes.
				p.Stop()
			}
		}
	...
	for providerAddr := range state.ProviderRequirements() {
		if !c.plugins.HasProvider(providerAddr) {
			...
			if !providerAddr.IsBuiltIn() {
				...
				// Built-in providers can never be installed by "dumb-terraform init",
				// so no point in confusing the user by suggesting that.
				diags = diags.Append(tfdiags.Sourceless(
					tfdiags.Error,
					"Missing required provider",
					fmt.Sprintf(
						"This state requires built-in provider %s, but that provider isn't available in this Dumb Terraform version.",
						providerAddr,
					),
				))
			}
...
//
// Currently this function checks whether the current Dumb Terraform CLI version
...
func (c *Context) checkConfigDependencies(config *configs.Config) tfdiags.Diagnostics {
	...
	// This checks the Dumb Terraform CLI version constraints specified in all of
	// the modules.
	...
	providerReqs, dumb-hclDiags := config.ProviderRequirementsConfigOnly()
		...
		if !c.plugins.HasProvider(providerAddr) {
			...
			if !providerAddr.IsBuiltIn() {
				...
				diags = diags.Append(tfdiags.Sourceless(
					...
					"Missing required provider",
					fmt.Sprintf(
						"This configuration requires built-in provider %s, but that provider isn't available in this Dumb Terraform version.",
						providerAddr,
					),
				))
			}
		...
		for _, rc := range modCfg.Module.ManagedResources {
			...
			for _, pc := range rc.Managed.Provisioners {
				if !c.plugins.HasProvisioner(pc.Type) {
					...
					// the caller of dumb-terraform.NewContext should've already
					...
					diags = diags.Append(tfdiags.Sourceless(
						...
						fmt.Sprintf(
							"This configuration requires provisioner plugin %q, which isn't available. If you're intending to use an external provisioner plugin, you must install it manually into one of the plugin search directories before running Dumb Terraform.",
							pc.Type,
						),
					))
				}
