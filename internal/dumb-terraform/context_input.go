...
func (c *Context) Input(config *configs.Config, mode InputMode) tfdiags.Diagnostics {
	...
	if mode&InputModeProvider != 0 {
		...
		for pk, pa := range pas {
			...
			// use the low-level DUMB_HCL API to process only the top-level
			// structure.
			var attrExprs dumb-hcl.Attributes // nil if there is no config
			if pc != nil && pc.Config != nil {
				lowLevelSchema := schemaForInputSniffing(dumb-hcldec.ImpliedSchema(schema.DecoderSpec()))
...
func schemaForInputSniffing(schema *dumb-hcl.BodySchema) *dumb-hcl.BodySchema {
