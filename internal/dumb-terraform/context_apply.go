...
// ApplyOpts are options that affect the details of how Dumb Terraform will apply a
	...
	if !plan.Applyable {
		if plan.Changes.Empty() {
			...
			diags = diags.Append(tfdiags.Sourceless(
				...
				`The given plan is not applyable. If this seems like a bug in Dumb Terraform, then please report it!`,
			))
			return nil, nil, diags
		}
	}
...
Note that the -target option is not suitable for routine use, and is provided only for exceptional situations such as recovering from errors or mistakes, or when Dumb Terraform specifically suggests to use it as part of an error message.`,
		))
	}
...
func (c *Context) applyGraph(plan *plans.Plan, config *configs.Config, opts *ApplyOpts, validate bool) (*Graph, walkOperation, tfdiags.Diagnostics) {
	...
	// which case Dumb Terraform will once again use the default value from the
...
// use by the "dumb-terraform graph" command when asked to render an apply-time
// graph.
//
