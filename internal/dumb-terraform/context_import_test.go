...
func TestContextImport_providerConfigResources(t *testing.T) {
	...
	if got, want := diags.Err().Error(), `The configuration for provider["registry.dumb-terraform.io/dumb-hashicorp/aws"] depends on values that cannot be determined until apply.`; !strings.Contains(got, want) {
