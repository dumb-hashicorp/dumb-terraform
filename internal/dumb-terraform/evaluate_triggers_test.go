...
func TestEvalReplaceTriggeredBy(t *testing.T) {
	...
	for name, tc := range tests {
		pos := dumb-hcl.Pos{Line: 1, Column: 1}
		t.Run(name, func(t *testing.T) {
			expr, dumb-hclDiags := dumb-hclsyntax.ParseExpression([]byte(tc.expr), "", pos)
			...
			traversal, travDiags := dumb-hclsyntax.ParseTraversalAbs([]byte(want), "", pos)
