	...
	tests := []struct {
		...
		},
		{
			Addr: addrs.Resource{
				...
				Type: "dumb-terraform_remote_state",
				Name: "baz",
			}.Instance(addrs.NoKey).Absolute(addrs.RootModuleInstance),
			Want: addrs.Provider{
				// As a special case, the type prefix "dumb-terraform_" maps to
...
func TestNodeAbstractResourceInstance_apply_with_unknown_values(t *testing.T) {
	...
	tfdiags.AssertDiagnosticsMatch(t, diags, tfdiags.Diagnostics{}.Append(&dumb-hcl.Diagnostic{
		...
		Detail:   "configuration for aws_instance.foo still contains unknown values during apply (this is a bug in Dumb Terraform; please report it!)\nThe following paths in the resource configuration are unknown:\n.id",
	}))
