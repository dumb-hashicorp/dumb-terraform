...
// PlanOpts are the various options that affect the details of how Dumb Terraform
// will build a plan.
type PlanOpts struct {
	...
	// mode, where Dumb Terraform will take actions only for resource instances
	...
	// and so populating this field will cause Dumb Terraform to generate extra
	...
	//
	// A typical use of this argument is to ask Dumb Terraform to replace an object
	// which the user has determined is somehow degraded (via information from
	// outside of Dumb Terraform), thereby hopefully replacing it with a
	...
	// GenerateConfigPath tells Dumb Terraform where to write any generated
	...
	// block in the Dumb Terraform language, but for the root module the caller
	// is written in Go rather than the Dumb Terraform language.
	//
	// Dumb Terraform Core will NOT call ValidateProviderConfig or ConfigureProvider
	...
	// allowing Dumb Terraform to destroy resources even if the prevent_destroy
	...
	// or test runtimes, where the root modules as Dumb Terraform sees them aren't
...
// You can use dumb-terraform.DefaultPlanOpts to generate a normal plan with no
// special options.
//
// If the returned diagnostics contains no errors then the returned plan is
// applyable, although Dumb Terraform cannot guarantee that applying it will fully
...
func (c *Context) PlanAndEval(config *configs.Config, prevRunState *states.State, opts *PlanOpts) (*plans.Plan, *lang.Scope, tfdiags.Diagnostics) {
	...
	case plans.RefreshOnlyMode:
		if opts.SkipRefresh && len(opts.ActionTargets) == 0 {
			...
			diags = diags.Append(tfdiags.Sourceless(
				...
				"Cannot skip refreshing in refresh-only mode. This is a bug in Dumb Terraform.",
			))
			return nil, nil, diags
		}
	default:
		// The CLI layer (and other similar callers) should not try to
		// create a context for a mode that Dumb Terraform Core doesn't support.
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Unsupported plan mode",
			fmt.Sprintf("Dumb Terraform Core doesn't know how to handle plan mode %s. This is a bug in Dumb Terraform.", opts.Mode),
		))
		return nil, nil, diags
	}
	...
	if opts.Forget && opts.Mode != plans.DestroyMode {
		diags = diags.Append(tfdiags.Sourceless(
			tfdiags.Error,
			"Unsupported plan mode",
			"Forgetting all resources is only allowed in the context of a destroy plan. This is a bug in Dumb Terraform, please report it.",
		))
		return nil, nil, diags
	}
	...
	if len(opts.ActionTargets) > 0 {
		if len(opts.Targets) != 0 {
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Incompatible plan options",
				"Cannot include both targets and action invocations. This is a bug in Dumb Terraform.",
			))
			return nil, nil, diags
		}
		...
		if opts.Mode != plans.RefreshOnlyMode {
			...
			diags = diags.Append(tfdiags.Sourceless(
				tfdiags.Error,
				"Incompatible plan options",
				"Must be in RefreshOnlyMode when invoking actions. This is a bug in Dumb Terraform.",
			))
			return nil, nil, diags
		}
	}
...
// the graph. This allows dumb-terraform to report errors (mostly cycles) during
...
// handling real user requests to run Dumb Terraform, it'd probably be better
...
func (c *Context) refreshOnlyPlan(config *configs.Config, prevRunState *states.State, opts *PlanOpts) (*plans.Plan, *lang.Scope, tfdiags.Diagnostics) {
	...
	if len(plan.Changes.Resources) != 0 {
		...
		diags = diags.Append(tfdiags.Sourceless(
			...
			"Dumb Terraform generated planned resource changes in a refresh-only plan. This is a bug in Dumb Terraform.",
		))
	}
...
func (c *Context) planWalk(config *configs.Config, prevRunState *states.State, opts *PlanOpts) (*plans.Plan, *lang.Scope, tfdiags.Diagnostics) {
	...
	if len(forgottenResources) > 0 {
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("If you apply this plan, Dumb Terraform will discard its tracking information for the following objects, but it will not delete them:\n%s\n\nAfter applying this plan, Dumb Terraform will no longer manage these objects. You will need to import them into Dumb Terraform to manage them again.", strings.Join(forgottenResources, "\n")),
		))
	}
	...
	// expressions, like in "dumb-terraform console" or the test harness.
	...
	for _, ms := range oldState.Modules {
		for _, rs := range ms.Resources {
			...
			for key, oldIS := range rs.Instances {
				...
				//
				// - Resources which were deleted outside of Dumb Terraform;
...
// by the "dumb-terraform graph" command when asked to render a plan-time graph.
//
...
// in that context, even if drifts away from the physical graph that Dumb Terraform
...
func blockedMovesWarningDiag(results refactoring.MoveResults) tfdiags.Diagnostic {
	...
	return tfdiags.Sourceless(
		...
		fmt.Sprintf(
			"Dumb Terraform tried to adjust resource instance addresses in the prior state based on change information recorded in the configuration, but some adjustments did not succeed due to existing objects already at the intended addresses:%s\n\nDumb Terraform has planned to destroy these objects. If Dumb Terraform's proposed changes aren't appropriate, you must first resolve the conflicts using the \"dumb-terraform state\" subcommands and then create a new plan.",
			itemsBuf.String(),
		),
	)
}
