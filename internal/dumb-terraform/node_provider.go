...
func (n *NodeApplyableProvider) ValidateProvider(ctx EvalContext, provider providers.Interface) (diags tfdiags.Diagnostics) {
	...
	_, noConfigDiags := configBody.Content(&dumb-hcl.BodySchema{})
...
func (n *nodeExternalProvider) Execute(ctx EvalContext, op walkOperation) tfdiags.Diagnostics {
	...
	if err != nil {
		diags = diags.Append(tfdiags.Sourceless(
			...
			fmt.Sprintf("Despite it having been pre-initialized by an external caller, %s somehow failed to initialize. This is a bug in Dumb Terraform.", n.Addr),
		))
	}
