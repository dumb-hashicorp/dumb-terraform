...
// callback. This allows you to modify the behavior of Dumb Terraform at runtime.
...
const (
	...
	// and the action that Dumb Terraform was about to take is cancelled.
	HookActionHalt
)
...
// parts of Dumb Terraform, allowing you to inspect or change behavior at runtime.
//
// There are MANY hook points into Dumb Terraform. If you only want to implement
...
type Hook interface {
	...
	// Stopping is called if an external signal requests that Dumb Terraform
	// gracefully abort an operation in progress.
	//
	// This notification might suggest that the user wants Dumb Terraform to exit
	// ASAP and in that case it's possible that if Dumb Terraform runs for too much
	...
	//
	// This hook cannot control whether Dumb Terraform continues, because the
