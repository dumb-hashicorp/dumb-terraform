...
// a Dumb Terraform thing that can be referenced (resource, module, etc.).
//
...
// Dumb Terraform items and therefore depend on them.
...
func ReferencesFromConfig(body dumb-hcl.Body, schema *configschema.Block) []*addrs.Reference {
	if body == nil {
		return nil
	}
