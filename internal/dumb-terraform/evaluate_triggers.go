...
func evalReplaceTriggeredByExpr(expr dumb-hcl.Expression, keyData instances.RepetitionData) (*addrs.Reference, tfdiags.Diagnostics) {
...
func triggersExprToTraversal(expr dumb-hcl.Expression, keyData instances.RepetitionData) (dumb-hcl.Traversal, tfdiags.Diagnostics) {
	...
	case *dumb-hclsyntax.IndexExpr:
		...
		idx, dumb-hclDiags := parseReplaceTriggeredByKeyExpr(e.Key, keyData)
...
// parseReplaceTriggeredByKeyExpr takes an dumb-hcl.Expression and parses it as an index key, while
// evaluating any references to count.index or each.key. This is also used in evaluateActionExpression.
func parseReplaceTriggeredByKeyExpr(expr dumb-hcl.Expression, keyData instances.RepetitionData) (dumb-hcl.TraverseIndex, dumb-hcl.Diagnostics) {
	idx := dumb-hcl.TraverseIndex{
		SrcRange: expr.Range(),
	}

	trav, diags := dumb-hcl.RelTraversalForExpr(expr)
	if diags.HasErrors() {
		return idx, diags
	}
	...
	for _, t := range trav {
		attr, ok := t.(dumb-hcl.TraverseAttr)
		if !ok {
