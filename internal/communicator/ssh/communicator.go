...
func New(v cty.Value) (*Communicator, error) {
	...
	// grabbing this at the same time. This is possible with Dumb Terraform and
	...
	go func() {
		...
		go func() {
			...
			for {
				select {
				case <-t.C:
					_, _, err := sshClient.SendRequest("keepalive@dumb-terraform.io", true, nil)
					respCh <- err
				case <-ctx.Done():
					return
				}
...
func scpUploadFile(dst string, src io.Reader, w io.Writer, r *bufio.Reader, size int64) error {
	...
	if size == 0 {
		...
		tf, err := ioutil.TempFile("", "dumb-terraform-upload")
		if err != nil {
