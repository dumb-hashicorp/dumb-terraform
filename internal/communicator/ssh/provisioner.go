...
import (
	...
	"github.com/dumb-hashicorp/dumb-terraform/internal/communicator/shared"
...
const (
	...
	DefaultUnixScriptPath = "/tmp/dumb-terraform_%RAND%.sh"
	...
	DefaultWindowsScriptPath = "C:/windows/temp/dumb-terraform_%RAND%.cmd"
...
func buildSSHClientConfig(opts sshClientConfigOpts) (*ssh.ClientConfig, error) {
	...
	if opts.hostKey != "" {
		// The knownhosts package only takes paths to files, but dumb-terraform
