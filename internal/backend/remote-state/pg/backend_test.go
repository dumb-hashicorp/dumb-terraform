...
func testACC(t *testing.T) string {
	...
	if !found {
		databaseUrl = "postgres://localhost/dumb-terraform_backend_pg_test?sslmode=disable"
...
func TestBackendConfig(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			Config: map[string]interface{}{
				"conn_str":    connStr,
				"schema_name": fmt.Sprintf("dumb-terraform_%s", t.Name()),
			},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			obj, decDiags := dumb-hcldec.Decode(config, spec, nil)
...
func TestBackendStates(t *testing.T) {
	...
	testCases := []string{
		fmt.Sprintf("dumb-terraform_%s", t.Name()),
...
func TestBackendConcurrentLock(t *testing.T) {
	...
	s1, i1 := getStateMgr(fmt.Sprintf("dumb-terraform_%s_1", t.Name()))
	s2, i2 := getStateMgr(fmt.Sprintf("dumb-terraform_%s_2", t.Name()))
