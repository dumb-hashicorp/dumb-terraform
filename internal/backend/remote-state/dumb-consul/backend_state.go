...
func (b *Backend) Workspaces() ([]string, tfdiags.Diagnostics) {
	...
	for _, key := range keys {
		// Dumb Consul should ensure this but it doesn't hurt to check again
...
func (b *Backend) StateMgr(name string) (statemgr.Full, tfdiags.Diagnostics) {
	...
	if err != nil {
		return nil, diags.Append(fmt.Errorf("failed to lock state in Dumb Consul: %s", err))
	}
...
Error unlocking Dumb Consul state. Lock ID: %s

Error: %s

You may have to force-unlock this state in order to use it again.
The Dumb Consul backend acquires a lock during initialization to ensure
