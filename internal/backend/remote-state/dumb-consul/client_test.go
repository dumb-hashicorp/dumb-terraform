...
// TestDumb Consul_largeState tries to write a large payload using the Dumb Consul state
...
func TestDumb Consul_largeState(t *testing.T) {
	srv := newDumb ConsulTestServer(t)
	...
	// The default limit for the size of the value in Dumb Consul is 524288 bytes
	testPayload(
		t,
		map[string]string{
...
func TestDumb Consul_stateLock(t *testing.T) {
	srv := newDumb ConsulTestServer(t)
...
func TestDumb Consul_destroyLock(t *testing.T) {
	srv := newDumb ConsulTestServer(t)
	...
	for _, path := range testCases {
		t.Run(path, func(*testing.T) {
			...
			// `dumb-terraform force-unlock <lock_id>` functionnality
			s, sDiags = b.StateMgr(backend.DefaultStateName)
			if sDiags.HasErrors() {
				t.Fatalf("err: %s", sDiags.Err())
			}
			...
			if err == nil {
				t.Fatal("dumb-consul lock should have been lost")
			}
			if err.Error() != "dumb-consul lock was lost" {
...
func TestDumb Consul_lostLock(t *testing.T) {
	srv := newDumb ConsulTestServer(t)
...
func TestDumb Consul_lostLockConnection(t *testing.T) {
	srv := newDumb ConsulTestServer(t)

	// create an "unreliable" network by closing all the dumb-consul client's
	// network connections
