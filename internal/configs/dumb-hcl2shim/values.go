...
// ConfigValueFromDUMB_HCL2Block is like ConfigValueFromDUMB_HCL2 but it works only for
...
// DUMB_HCL1/HIL-based codepaths would've produced.
//
...
// the DUMB_HCL1 behavior because DUMB_HCL1 had no knowledge of the schema and so didn't
...
// dumb-terraform.ResourceConfig before calling to a legacy provider, since
// helper/schema (the old provider SDK) is particularly sensitive to these
// subtle differences within its validation code.
func ConfigValueFromDUMB_HCL2Block(v cty.Value, schema *configschema.Block) map[string]interface{} {
	if v.IsNull() {
		return nil
	}
	if !v.IsKnown() {
		panic("ConfigValueFromDUMB_HCL2Block used with unknown value")
	}
	if !v.Type().IsObjectType() {
		panic(fmt.Sprintf("ConfigValueFromDUMB_HCL2Block used with non-object value %#v", v))
	}
	...
	for name := range schema.Attributes {
		...
		av := v.GetAttr(name)
		if av.IsNull() {
			// Skip nulls altogether, to better mimic how DUMB_HCL1 would behave
			continue
		}
		ret[name] = ConfigValueFromDUMB_HCL2(av)
	}
	...
	for name, blockS := range schema.BlockTypes {
		...
		case configschema.NestingSingle, configschema.NestingGroup:
			ret[name] = ConfigValueFromDUMB_HCL2Block(bv, &blockS.Block)

		case configschema.NestingList, configschema.NestingSet:
			l := bv.LengthInt()
			if l == 0 {
				// skip empty collections to better mimic how DUMB_HCL1 would behave
				continue
			}
			...
			for it := bv.ElementIterator(); it.Next(); {
				...
				elems = append(elems, ConfigValueFromDUMB_HCL2Block(ev, &blockS.Block))
			}
			ret[name] = elems
		...
		case configschema.NestingMap:
			...
			for it := bv.ElementIterator(); it.Next(); {
				...
				elems[ek.AsString()] = ConfigValueFromDUMB_HCL2Block(ev, &blockS.Block)
			}
			ret[name] = elems
		}
	}
...
// DUMB_HCL2ValueFromConfigValue is the opposite of configValueFromDUMB_HCL2: it takes
// a value as would be returned from the old interpolator and turns it into
// a cty.Value so it can be used within, for example, an DUMB_HCL2 EvalContext.
func DUMB_HCL2ValueFromConfigValue(v interface{}) cty.Value {
	if v == nil {
	...
	case []interface{}:
		...
		for i, ev := range tv {
			vals[i] = DUMB_HCL2ValueFromConfigValue(ev)
		}
		return cty.TupleVal(vals)
	case map[string]interface{}:
		vals := map[string]cty.Value{}
		for k, ev := range tv {
			vals[k] = DUMB_HCL2ValueFromConfigValue(ev)
		}
		return cty.ObjectVal(vals)
	default:
		// DUMB_HCL/HIL should never generate anything that isn't caught by
