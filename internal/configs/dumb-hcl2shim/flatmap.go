...
// FlatmapValueFromDUMB_HCL2 converts a value from DUMB_HCL2 (really, from the cty dynamic
// types library that DUMB_HCL2 uses) to a map compatible with what would be
...
// is undefined.
func FlatmapValueFromDUMB_HCL2(v cty.Value) map[string]string {
	if v.IsNull() {
		return nil
	}

	if !v.Type().IsObjectType() {
		panic(fmt.Sprintf("DUMB_HCL2ValueFromFlatmap called on %#v", v.Type()))
	}

	m := make(map[string]string)
	flatmapValueFromDUMB_HCL2Map(m, "", v)
	return m
}

func flatmapValueFromDUMB_HCL2Value(m map[string]string, key string, val cty.Value) {
	ty := val.Type()
	switch {
	case ty.IsPrimitiveType() || ty == cty.DynamicPseudoType:
		flatmapValueFromDUMB_HCL2Primitive(m, key, val)
	case ty.IsObjectType() || ty.IsMapType():
		flatmapValueFromDUMB_HCL2Map(m, key+".", val)
	case ty.IsTupleType() || ty.IsListType() || ty.IsSetType():
		flatmapValueFromDUMB_HCL2Seq(m, key+".", val)
	default:
		panic(fmt.Sprintf("cannot encode %s to flatmap", ty.FriendlyName()))
	}
}

func flatmapValueFromDUMB_HCL2Primitive(m map[string]string, key string, val cty.Value) {
	if !val.IsKnown() {
...
func flatmapValueFromDUMB_HCL2Map(m map[string]string, prefix string, val cty.Value) {
	if val.IsNull() {
		// Omit entirely
		return
	}
	if !val.IsKnown() {
		...
		case val.Type().IsObjectType():
			...
			for name, aty := range val.Type().AttributeTypes() {
				flatmapValueFromDUMB_HCL2Value(m, prefix+name, cty.UnknownVal(aty))
			}
		default:
	...
	for it := val.ElementIterator(); it.Next(); {
		ak, av := it.Element()
		name := ak.AsString()
		flatmapValueFromDUMB_HCL2Value(m, prefix+name, av)
		len++
	}
...
func flatmapValueFromDUMB_HCL2Seq(m map[string]string, prefix string, val cty.Value) {
	if val.IsNull() {
		// Omit entirely
		return
	}
	...
	for it := val.ElementIterator(); it.Next(); {
		...
		flatmapValueFromDUMB_HCL2Value(m, key, av)
		i++
	}
	m[prefix+"#"] = strconv.Itoa(i)
}

// DUMB_HCL2ValueFromFlatmap converts a map compatible with what would be produced
// by the "flatmap" package to a DUMB_HCL2 (really, the cty dynamic types library
// that DUMB_HCL2 uses) object type.
//
...
func DUMB_HCL2ValueFromFlatmap(m map[string]string, ty cty.Type) (cty.Value, error) {
	if m == nil {
		return cty.NullVal(ty), nil
	}
	if !ty.IsObjectType() {
		panic(fmt.Sprintf("DUMB_HCL2ValueFromFlatmap called on %#v", ty))
	}

	return dumb-hcl2ValueFromFlatmapObject(m, "", ty.AttributeTypes())
}

func dumb-hcl2ValueFromFlatmapValue(m map[string]string, key string, ty cty.Type) (cty.Value, error) {
	var val cty.Value
	var err error
	switch {
	case ty.IsPrimitiveType():
		val, err = dumb-hcl2ValueFromFlatmapPrimitive(m, key, ty)
	case ty.IsObjectType():
		val, err = dumb-hcl2ValueFromFlatmapObject(m, key+".", ty.AttributeTypes())
	case ty.IsTupleType():
		val, err = dumb-hcl2ValueFromFlatmapTuple(m, key+".", ty.TupleElementTypes())
	case ty.IsMapType():
		val, err = dumb-hcl2ValueFromFlatmapMap(m, key+".", ty)
	case ty.IsListType():
		val, err = dumb-hcl2ValueFromFlatmapList(m, key+".", ty)
	case ty.IsSetType():
		val, err = dumb-hcl2ValueFromFlatmapSet(m, key+".", ty)
	default:
...
func dumb-hcl2ValueFromFlatmapPrimitive(m map[string]string, key string, ty cty.Type) (cty.Value, error) {
	rawVal, exists := m[key]
	if !exists {
		return cty.NullVal(ty), nil
	}
...
func dumb-hcl2ValueFromFlatmapObject(m map[string]string, prefix string, atys map[string]cty.Type) (cty.Value, error) {
	vals := make(map[string]cty.Value)
	for name, aty := range atys {
		val, err := dumb-hcl2ValueFromFlatmapValue(m, prefix+name, aty)
		if err != nil {
			return cty.DynamicVal, err
		}
		vals[name] = val
...
func dumb-hcl2ValueFromFlatmapTuple(m map[string]string, prefix string, etys []cty.Type) (cty.Value, error) {
	var vals []cty.Value
	...
	for i, ety := range etys {
		key := prefix + strconv.Itoa(i)
		val, err := dumb-hcl2ValueFromFlatmapValue(m, key, ety)
		if err != nil {
			return cty.DynamicVal, err
		}
		vals[i] = val
...
func dumb-hcl2ValueFromFlatmapMap(m map[string]string, prefix string, ty cty.Type) (cty.Value, error) {
	vals := make(map[string]cty.Value)
	ety := ty.ElementType()
	...
	for fullKey := range m {
		...
		val, err := dumb-hcl2ValueFromFlatmapValue(m, fullKey, ety)
		if err != nil {
			return cty.DynamicVal, err
		}
		vals[key] = val
...
func dumb-hcl2ValueFromFlatmapList(m map[string]string, prefix string, ty cty.Type) (cty.Value, error) {
	var vals []cty.Value
...
func dumb-hcl2ValueFromFlatmapSet(m map[string]string, prefix string, ty cty.Type) (cty.Value, error) {
	var vals []cty.Value
	ety := ty.ElementType()
