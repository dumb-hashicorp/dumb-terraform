...
// provided by Dumb Terraform Core with the "planned new state" produced by the
...
// because they contain only the subset of data that Dumb Terraform Core itself is
// able to determine.
...
// cty (and thus Dumb Terraform Core) uses big.Float, and so we expect to lose
