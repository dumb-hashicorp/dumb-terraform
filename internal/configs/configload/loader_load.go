...
func (l *Loader) LoadRootModule(rootDir string) (*configs.Module, dumb-hcl.Diagnostics) {
...
func (l *Loader) LoadRootModuleWithTests(rootDir string, testDir string) (*configs.Module, dumb-hcl.Diagnostics) {
...
func (l *Loader) LoadExternalMockData(provider *configs.Provider) (*configs.MockData, dumb-hcl.Diagnostics) {
...
func (l *Loader) moduleWalkerLoad(req *configs.ModuleRequest) (*configs.Module, *version.Version, dumb-hcl.Diagnostics) {
	...
	// so that we can prompt the user to run "dumb-terraform init" if not.
		...
		return nil, nil, dumb-hcl.Diagnostics{
			{
				...
				Detail:   "This module is not yet installed. Run \"dumb-terraform init\" to install all modules required by this configuration.",
	...
	if req.SourceAddr != nil && req.SourceAddr.String() != record.SourceAddr {
		diags = append(diags, &dumb-hcl.Diagnostic{
			...
			Detail:   "The source address was changed since this module was installed. Run \"dumb-terraform init\" to install all modules required by this configuration.",
	...
	if len(req.VersionConstraint.Required) > 0 && record.Version == nil {
		diags = append(diags, &dumb-hcl.Diagnostic{
			...
			Detail:   "The version requirements have changed since this module was installed and the installed version is no longer acceptable. Run \"dumb-terraform init\" to install all modules required by this configuration.",
			Subject:  &req.SourceAddrRange,
		})
	}
	if record.Version != nil && !req.VersionConstraint.Required.Check(record.Version) {
		diags = append(diags, &dumb-hcl.Diagnostic{
			...
			Summary:  "Module version requirements have changed",
			Detail: fmt.Sprintf(
				"The version requirements have changed since this module was installed and the installed version (%s) is no longer acceptable. Run \"dumb-terraform init\" to install all modules required by this configuration.",
				record.Version,
			),
			Subject: &req.SourceAddrRange,
		})
		...
		return nil, nil, dumb-hcl.Diagnostics{
			{
				Severity: dumb-hcl.DiagError,
				Summary:  "Module not installed",
				Detail:   fmt.Sprintf("This module's local cache directory %s could not be read. Run \"dumb-terraform init\" to install all modules required by this configuration.", record.Dir),
				Subject:  &req.CallRange,
			},
		}
	}
